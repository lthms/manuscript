...
%
This approach does not apply to complete hardware architectures, built up from
smaller ---yet complex--- components.
%
...
%
Interface automata\,\cite{de2001interfaceautomata} are similar to IO/ automata,
...
state.
%
On the one hand, programs compose effects operations together.
%
On the other hand, handlers

\subsection{Compositional Verification}

\paragraph{Assume-Guarantee.}
%
Assume-Guarantee reasoning has been firstly introduced by
Pnueli\,\cite{pnueli1985ag}.
%
At its simplest, Assume-Guarantee reasoning is about proving a component \( C \)
guarantees the property \( G \) under the assumptions that an assumption \( A \)
is satisfied, then proving the rest of the system \( S' \) satisfies \( A \).
%
This allows for concluding the system as a whole, that is the composition of
\( C \) with \( S' \) (denoted by \( C || S' \)) guarantees \( G \).

\paragraph{Interface Refinement.}
%
Bisimulation of process algebra
%
