...
In this Chapter, we give a formal definition of HSE mechanisms.
...
\begin{definition}[Transitions]
  We denote by $\mathcal{T}(\Sigma)$ the set of triples which satisfy the
...
\begin{definition}[Runs]
  One particular execution of software components by the hardware architecture
  is modelled as a run of $\Sigma$.
  %
  A run is a sequence of transitions of~$\Sigma$, such that for two consecutive
  transitions, the resulting state of the first is the initial state of the
  next.

  We denote $\pathesLTS{Ez}$ the set of runs of the Computing Platform
  $\shortLTS{Ez}$, $\func{init}(\rho) \in H$ the initial state of a run $\rho$,
  and $\func{trans}(\rho) \in \powerset(\mathcal{T}(\Sigma))$ the set of
  transitions which occur during this run.
\end{definition}

\subsection{Software vs. Hardware Transitions}
...
%
Other hardware transitions may be provoked by other hardware components.
%
...
We now discuss how we can model the different security policies introduced in
Section~\ref{subsec:state:secu}.

\subsection{General Case}

\[
  P : \powerset(\pathesLTS{Ez}) \rightarrow \ac{prop}
\]

\subsection{Properties on Runs}

\[
  P : \pathesLTS{Ez} \rightarrow \ac{prop}
\]

\[
  \func{run\_property}_{P}(R) \triangleq \forall \rho \in R, P(\rho)
\]

\subsection{Safety Properties}

\[
  P : \mathcal{T}(\Sigma) \rightarrow \ac{prop}
\]

\[
  \func{safety\_property}_{P}(R) \triangleq \forall \rho \in R, \forall (h, ev,
  h') \in \mathcal{T}(\rho), P(h, ev, h')
\]
...
One of the specificities of HSE mechanisms is the sharing of the \ac{cpu}
...
\begin{definition}[HSE Mechanism]
  ...
  $\langle \Sigma, S, T, \func{context}, \func{inv}, \func{behaviour}
  \rangle$, such that
  %
  \begin{itemize}
...
%
\item When a software component (\emph{e.g.} a system software) tries to unlock
...
transformations which do not satisfy $\func{behavior}$.
%
...
untrusted and we make no assumption on their behavior.
...
\subsection{Run Compliance}

We say a run complies to a \ac{hse} mechanism definition when its initial state
satisfies the state requirements (\func{inv}) and each software transitions of
the run satisfies the software requirements (\func{behaviour}).
%
\begin{definition}[Compliant Runs]
  We write $\mathcal{C}(\Delta)$ for the set of the runs which comply with
  $\Delta$.
  %
  ...
  \[
    ...
    \mathcal{T}(\Sigma), l \in L_S \Rightarrow \func{behaviour}(h, l)
  \]
\end{definition}

\begin{lemma}[HSE Invariant Enforcement]
  For all $\rho \in \mathcal{C}(\Delta)$, we prove that \func{inv} is satisfied
  throughout compliant runs, that is
  %
  \[
    \forall (h, l, h') \in \mathcal{T}(\rho), inv(h) \wedge inv(h')
  \]

  \begin{proof}
    By definition of $\mathcal{C}(\Delta)$, we know the initial state of the run
    satisfies \func{inv}, and thanks to the first HSE Law, we can conclude the
    state after the first transition also satisfies \func{inv}.
    %
    We can generalize to the run by induction.
  \end{proof}
\end{lemma}
...
\begin{definition}[Correct HSE Mechanism]
  ...
  (denoted by $\Delta \sqsubseteq P$) if and only if the set of compliant runs
