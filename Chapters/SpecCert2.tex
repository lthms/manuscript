...
(Definition~\ref{definition:speccert:isolation}), and we defined a sub-policy
which focuses on the \ac{bios} protection (\ref{def:speccert:biospol}).
%
In this Chapter, we apply our approach to specify and verify the \ac{hse}
mechanism implemented by the \ac{bios} on x86 hardware architecture at runtime
and described in Section~\ref{sec:usecase:hse}.
%
The purpose of this \ac{hse} mechanisms is to provide an isolated execution
environment to the \ac{bios}, and therefore should enforce the so-called
