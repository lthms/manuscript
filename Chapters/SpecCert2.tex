...
overwrites its content during a transition. \TODO{Sauf erreur de ma part, cette notion de "owner" n'est définit nulle part formellement. elle devrait l'être} 
%
...
\TODO{Tu définis formellement le prototype de cette fonction mais pas vraiement la fonction elle même. Tu devrais le faire à partir de la définition de owner (et possiblement de la valeur du compteur programme). En outre, sauf erreur de ma part, l'exécution d'une instruction se traduit pas plusieurs transitions software. Du coup, le mapping renvoie la même liste pour chaque transition correspondant à l'exécution d'une instruction ou bien seulement pour la première? Il faudrait clarifier tout çà (une vraie définition formelle aiderait...)} 
...
With a hardware-software mapping and an event-software mapping \TODO{"Event-software mapping" c'est la même chose que "transition-software mapping"? Ne pas utiliser deux termes différents pour la même notion... }, we give a formal
...
\begin{definition}[Code Injection]
  ...
  code injection against~$y$.  with the execution of~$y$. \TODO{"with the execution of y" semble en trop...}
\end{definition}
...
\begin{example}[Application Code Injection]
  ...
  instructions which are owned by the OS. \TODO{Tu devrais noter avant (en dehors de l'exemple) que l'injection de code peut être légitime. cet example le montre.} 
\end{example}

\TODO{tu devrais placer un peu de texte entre les exemples, notamment pour les introduire et faire des transitions entre les exemples}
...
\begin{example}[SMRAM Cache Poisoning]
  ...
  the \texttt{SMBASE} register. \TODO{la description de cet exemple n'est pas très claire. Il faudrait mieux détailler l'injection et renvoyer éventuellement à une section décrivant  l'attaque pkus en détail si c'est le cas. En outre, le but final n'est pas très claire et très spécifique (contrairement à "escalade de privilèges" de l'exemple précédent, que tout le monde comprend.}
\end{example}
...
authorized to tamper with the execution \TODO{est-ce que "tamper with the execution" est synonyme de "make a code injection"? Dans ce cas, j'aimerais que tu n'utilises qu'un seul termes, et de préférence le plus précis. Tamper with laisse imagine d'autres formes d'interférences que l'on pourrait très légitimement vouloir interdir mais qui ne semble pas couverte par ton approche.} of another software component in the
...
\begin{corollary}
  ...
  \TODO{Dans ce corolaire, tu utilises la notation $ y \not\ge x$ alors qu'avant tu sembles utiliser la notation $x \rightarrowtail y$ pour la même chose. Soit j'ai loupé quelque chose soit il faut homogénéiser tes notations. En outre, ce corollaire est-il vraiment utile ici. Il semble trivial et on n'en voit pas trop l'intéret.}
...
\begin{example}[Software Stack Isolation]
  ...
  The definition of the corresponding $\func{context}$ \TODO{le style que tu utiiises pour les fonctions n'est pas très distinctif du texte classique. Quand un plus tu utilises des noms de fonctions qui sont des noms courant en anglais, comme "context", çà n'aide pas...} shall obey the following logic: when the
...
\TODO{Cette sous-section est vraiment très (trop) courte par rapport aux précédentes. Faut-il vraiment une sous-section dédiée?} 
...
in this Section. \TODO{"to define... we seek to define" fait une peu répétition}
