...
command-line application \texttt{lshw} lists about 30 hardware components,\PC{je
  ...
  composé de ``dozens of components'' :)} which include a hard drive, a
keyboard, a trackpad, an audio controller, an integrated graphic card. Several
...
%
In case of \emph{branch misprediction}, the erroneous computations performed by
the core shall be canceled like if they have never happened.
%
...
%
Therefore, a core decides the semantics of memory cell contents depending on the
context, \emph{i.e.} it can interpret the same binary sequence as an instruction
to execute then as an operand of an arithmetical operation.
...
\PC{après discussion d'hier, AMHA ici tu devrais faire une phrase pour
  ...
  \textit{via} l'espace d'adressage (mot important!) (le ``quoi'') et les
  manières d'y accéder (le ``comment''), sachant que l'accès à une même
  ressource peut être fait de deux manières différentes}
...
Other hardware components expose additional memory regions.\PC{techniquement, ce
  ...
  controller) qui expose la mémoire ou les registres du composant dans l'espace
  d'adressage global. Il serait utile de citer le nom \textit{memory-mapped I/O}
  ici --- c'est cité plus loin, mais en répétition}
%
...
%
In addition to this approach\PC{Proposal: ``Alternatively, peripheral memory and
  registers can be exposed to the system memory address space using \emph{memory
    mapped \IOs}.''

  Le ``Alternatively'' est plus clair que ``In addition to this approach''} ,
x86 \acp{cpu} are also capable of handling \emph{memory-mapped} \IOs, where the
memories exposed by the peripherals are mapped within the system memory address
space.
%
\PC{Phrase suivante inutile IMHO} That is, the system memory is no longer
limited to the \ac{dram} only.
%
...
\PC{saut de ligne: cette idée est importante, mais elle est noyée dans le
  paragraphe précédent. Il me semble aussi que ça part trop vite dans un exemple
  détaillé

  AMHA, il faudrait détailler plus l'idée générale, en expliquant le fait qu'une
  même zone mémoire ou registre d'un périphérique peut être lue ou modifiée par
  MMIO \textbf{et} PMIO, ce qui va notamment complexifier la modélisation, et
  réduire un peu l'exemple} The two approaches are complementary.
%
...
\PC{Je comprends l'idée de l'exemple, mais je ne suis pas persuadé que la
  fonction de calcul apporte quelque chose.  Tu pourrais plutôt prendre un
  registre au hasard dans la figure et expliquer comment on y accède
  (relativement au BAR ---qui peut être modifé---, ou \textit{via} PMIO),
  \textbf{en citant la figure 2.2}} A configuration register exposed by a given
PCI device identified as \texttt{bus:device:function} at the offset \( o \) of
its configuration space is addressed by the value
%
\[
...
%
When a core handles an interrupt, it saves its current context
inside the \ac{dram}, then starts executing the corresponding interrupt handler.
%
...
\PC{supprime le ``In addition'', et fais un paragraphe avec un titre} In
addition, Intel has implemented several out-of-band management technologies for
its products.
%
