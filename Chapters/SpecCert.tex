...
the security policy enforcement or finding restrictions over trusted
...
\begin{example}[Memory Flash Lockdown Compliance]
  ...
  %
  \GH{cet exemple n'illustre que l'état initial. Il faudrait dire en outre que
    lorsqu'une exécution est exécutée en SMM elle respecter les software req...}
\end{example}
...
%
\GH{pourquoi? Il faudrait expliquer rapidement pourquoi c'est bien le cas.}
%
...
instruction responsible to implement it.
%
...
traces where a given software component prevents another software component to
correctly implement its functional specification.
%
\GH{Cette phrase n'est pas très claire. Il faudrait développer en NOMMANT les
  choses. Un software component s1....}
%
...
\begin{definition}[Transition-Software Mapping]
  ...
  of owners of the fetched instructions during this transition. \GH{fetched
    instruction n'est pas défini clairement.}
\end{definition}
...
%
\GH{Cette phrase est ambigüe. Est-ce que c'est le fetch qui a lieu durant la
  software transition? J'avais cru comprendre que l'exécution d'une instruction
  pouvait correspondre à plusieurs "software transition". Du coup, pour chacune
  des ces transition, fetched renvoie la même valeur? Ou pas? Ce n'est pas
  claire}
...
\GH{attention avec ce claim un peu fort. Tu affirmes haut et fort qu'il te faut
  un modèle générique de x86 sauf que juste après, en section 5, tu vas annoncer
  que tu as fait un modèle réduit avec seulement les features qui
  t'intéressaient pour modéliser la SMM...}
%
