...
raises an interrupt.
%
In this scenario, only one software transition occurs ---when the core reads
...
between the core and a collection of peripherals, including the flash
memory.
%
...
mean it is not possible to implement both at the same time.
%
...
% \subsection*{}
%
% In this Section, we introduced our theory, from the definition to one \ac{hse}
% mechanism, up to how we can reason about compositions of \ac{hse} mechanisms.
%%
% In the next Section, we give a first example of how we can use our theory with
% a case study.
...
Therefore, a straightforward attack path attackers can leverage to defeat a
correct \ac{hse} mechanism is modifying trusted software components programs
thanks to illegitimate code injection (\ref{subsec:speccert:tampering}).
%
