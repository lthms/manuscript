...
to model and verify hardware and software systems, and where our approach
stands with respect to previous work (Section~\ref{sec:sota:formalisms}).
%
...
\begin{example}[Airlock Safety and Liveness Properties]
  ...
  both doors are closed), we verify this specification is correct with respect to
  ...
  \begin{proof}
    ...
    %
    Case 1, both doors are closed, that is \( d_1 = d_2 = \mathtt{closed} \).  The
...
\ac{hse} mechanism are outside of the scope of existing models, which is why we
developed our own.
%
...
% \end{inparaenum}
%%
% It would require a tremendous effort to be developed, reviewed and
...
%\GH{On ne comprend pas pourquoi tu nous parles subitement de ces deux approches
%  (et pourquoi tu n'en n'a pas parlé dans la section précédente)}
%
...
Monads have a reputation not to compose very
