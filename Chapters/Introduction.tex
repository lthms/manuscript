...
\item [Firmware/BIOS]
  ...
  several reasons.
  %
  \PC{To begin with}{For starters} updating software is easier than updating hardware, as the latter
  ...
  qui rendent la lecture un peu difficile.
  Il faudrait lisser un peu}
  %
\item [System Software]
  \PC{Tu parle bien de l'OS ? Pourquoi ne pas l'appeler par son nom, ou au
  minimum dire que le System Software est l'OS dans la plupart (tous?) des cas ?}
  %
...
\item [Applications]
  %
  Applications leverages the high-level interface exposed by \PC{(un peu
partout): \textbf{the} system software} system software in
...
the applications form one single layer\PC{discutable, il y a bien des layers
différents, la différence est surtout qu'ils ont les mêmes privilèges
$\rightarrow$ ``form one single software component'' ?}.
%
...
From a security perspective,\PC{layers are often used for privilege separation,
and} each layer is often more privileged than its
successors.
%
...
backdoors are no fantasy\PC{slang}\,\cite{yang2016a2}, the \ac{bios} has been used as an
...
\PC{mot inutile ?}{execution}, with respect to a targeted security policy.
%
...
\PC{le paragraphe suivant est un peu confus: tu parle de différente manières
de restreindre, mais la phrase 3 commence par un ``in a nutshell'' qui
est une idée générique (et pas une de ces manières), après tu as un exemple,
etc.}
...
\PC{(transition) AMHA, il manque une phrase ici pour dire pourquoi tu as besoin de la
définition qui suit. QQch comme ``To achieve security, each layer has to know and
use properly the security mechanisms offered by the underlying layers'' ?}
...
\begin{definition}[Hardware-based Security Enforcement]
  ...
  security policy.\PC{``the class of mechanisms where a \ldots component
  configures the hardware'' ça fait un peu bizarre à la lecture}
\end{definition}
...
\end{inparaenum}
%
Both \PC{requirements} remain challenging.

\paragraph{Software Errors.}
%
Vulnerabilities in \ac{hse} mechanism implementations by trusted software
components are often due to some misuse of hardware features.\PC{une citation
serait bienvenue}
%
Many facts can explain this state of things.
%
In the past, \PC{most} lower-level pieces of software, such as firmware components,
\PC{have (c'est un constat)}{may}
...
responsible.
%
While \PC{a computer is}{one computer can be} made of dozens of components, software developers have
...
\PC{circumvented}{compromised} due to
...
%
\PC{éviter ``the latter'' ici, juste dire ``These new features should not
interfere''}
...
%
\PC{(style haché)fusionner avec la phrase précédente: ``protected \ldots
based on a particular hardware interrupt''}
This protection relies on a particular hardware interrupt.
%
This did not prevent Intel to introduce TXT\,\cite{intel2015txt},\PC{on a
l'impression que l'erreur a été d'introduire TXT! il faudrait mettre le problème
en début de phrase} a novel
...
\begin{definition}[Compositional Attack]
  ...
  component is working as expected in isolation\PC{separately ?}, yet their composition creates
...
%
Compositional attacks are \PC{often} due to a flaw in the specifications of the computing
platform.
%
...
%
For each novel hardware feature introduced, it would be\PC{is?} necessary to check that
...
developers, in the form of a list of requirements to comply with \PC{and the
provided security properties}.
%
We believe these specifications can be\PC{are} a valuable addition to the existing
...
4842 pages long;
%
the Memory Controller Hub datasheet\,\cite{intel2009mch} is 430 \PC{pages?} long;
%
the Platform Controller Hub datasheet\,\cite{intel2012pch} is 988 pages long.
%
\PC{saut de ligne / nouveau paragraphe}
...
of products.\PC{et ? je ne comprends pas trop l'argument}
%
As a consequence, the more modular our models and proofs are, the more
practicable our approach becomes.
%
Otherwise, each modification of the hardware architecture can\PC{will} have an important
...
apply them to the x86 hardware architecture.\PC{we have decided to
systematically implement and evaluate them on the x86 architecture}
%
Because of its predominant position on the personal computer market, the x86
hardware architecture has been intensively\PC{extensively? (intensément vs
largement)} studied.
%
...
\item The extraction mechanism of Coq allows for transpiling formal
  ...
  their implementation, \PC{(suite phrase) mal formulé}this possibility is worth taking into account.
\end{itemize}
...
been disclosed over the past decade.\PC{attention, on comprend que tu vas
décrire tout x86 en détails}
%
