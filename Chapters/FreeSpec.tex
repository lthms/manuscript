...
Some transitions of {\scshape Minx86} imply that several state updates may
happen in sequence.
%
...
\]
%
\GH{A quoi correspondent t, s et r? Pourquoi 3 fonctions et pas plus? Ca semble
...
Besides, it emphasizes another important limitation of {\scshape Minx86}: it is
potentially subject to the so-called \emph{temporary violation
...
\emph{during} the transition. \GH{cette limitation est importante à noter (et
  elle est plus facile à comprendre telle que tu la décrit par la suite). En
  revanche, pour moi il s'agit d'une autre limitation que le côté monolytique,
  qui devrait donc se trouver dans un paragraphe dédié.}
%
For instance, if it exists a state \( h \) such that \( \neg r(h) \) or
\( \neg s(r(h)) \) holds true, then \( M \) is subject to the temporary
violation problem. \GH{Je ne comprends pas la condition car je ne vois pas à
  quoi correspond r et s}
%
...
\begin{example}[Temporary Violation Problem in an Airlock System]
  ...
  %
  We can define a transition system which is correct with respect to the usual
  safety property\,\footnote{At least one door should be close at all time.}
  \emph{and} includes these transitions.
...
violation of invariant, similarly to the Speed Racer
...
to modify the model another time. \GH{plutôt que 'une autre fois" il faudrait
  plutôt insister sur la nature et l'importance des changements que cela va
  nécessiter.}
%
...
\GH{je ne comprends pas "several change updates ---\emph{i.e.} one per state
  update". "change updates" me semble une répétition. Et je ne voit pas à quoi
  correspond un "state update". } and prove that \( \iota \) is preserved
throughout the transition, as illustrated in
Figure~\ref{freespec:figure:seqproof}. \GH{Quel est le langage utilisé dans
  cette figure? Galina? Il faudrait le signaler et commenter un peu cet extrait
  pour qu'il soit accessible au non-spécialiste. Ce n'est pas une mauvaise idée
  de mettre un tel code mais il faut que la plupart des relecteur puisse
...
%
\GH{On est un peu perdu. Tu as adopté cette approche dans SpecCert ou FreeSpec?}
%
In practice, we relied on a recurring pattern we call
``\texttt{remember};~\texttt{destruct};~\texttt{assert};~\texttt{apply}'', after
the {\scshape Ltac} tactics. \GH{Normalement l'objet de ce paragraphe est de
  décrire une limitation de MinX86. Or là j'ai l'impression que tu as glissé
  vers une description de ton approche, qui normalement est abordée après. Du
  coup, ce qui suit n'a peut-être pas sa place ici.} \GH{les termes "remember",
  "destruct", etc; sont-ils des termes à toi ou bien connu de la communauté Coq?
  A quoi correspondent-ils? Il me semble qu'il s'agisse de "tactique". Il
  faudrait le préciser ici}
%
We illustrate the use of this pattern with the proof goal pictured in
Figure~\ref{fig:freespec:entrypoint}.
%
We have extracted this goal directly from the SpecCert project; we have only
slightly simplified it so it can fit into the page.
%
\begin{enumerate}
\item Using \texttt{remember}, we can make the sequence of state updates more
  ...
  Figure~\ref{fig:freespec:entrypoint}). \GH{Pour moi qui ne suit pas un
    spécialiste, cette séquence ne me semble pas plus claire, je ne la vois dans
    aucun des deux figures. Il faudrait peut-être décrire par une phrase ou deux
    comment on doit interpréter/lire ces figures.}
  %
...
\item Using \texttt{destruct}, we can explore the alternative paths.
  ...
  %
  We explore both solution, though two complementary goals \GH{ne faut-il pas un
    "s" à solution? en outre, je ne comprend pas trop "though two complementary
    goals". Ne manquerait-il pas un mot?} (see the two simplified goals in
  Figure~\ref{fig:freespec:destruct})
  %
...
pattern structures the proof, but it remains cumbersome to use.
%
...
forgetting a problematic state update and yet being able to conclude the proof.
...
\begin{center}
  \begin{tikzpicture}
    ...
    % CPU
    \node [inner sep=16pt, draw, dashed, above=30pt of MC.center] (CPU) {CPU};%
