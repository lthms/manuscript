...
%
\thomasrk[inline]{Pose toi, prends bien le temps de justifier.}
%
...
\GH{je trouve que le choix de Coq mériterait d'être un peu mieux
  justifié. Pourquoi a-t-on besoin d'une logic d'ordre supérieur. Tu ne le
  justifie que sur un aspect assez anecdotique de ton travail (la composition
  des HSE).}
...
% validated, and the question of its applicability in a verification process
...
met. \GH{On ne comprend pas pourquoi tu nous parles subitement de ces deux
...
Because \( S \) uses \( T \), such a proof cannot but depend \GH{cannot but
  depend?} on how \( T \) operates.
%
...
\item we want to use well-established constructions in functional programming
  communities \GH{pourquoi? Il faut rappeler ici que Galina est un langage
    fonctionnel. Donc l'aspect fonctionnel t'es imposé par le choix de ton
    outil. Ca ne transparait pas assez!} in order to favor adoption, and
...
%
\GH{Il manque une transition avec la suite. On comprend pourquoi tu va nous
  parler de la composition dans les LTS et de l'algebre de processsus. Mais
  alors, pourquoi va tu nous parler d'une troisième chose après?}  We then
explain how we take advantage of a functional programming language \GH{encore
  une fois, tu mélanges la description de ton approche avec l'état de
  l'art. "how we take advantage" laisse penser que tu décrit ton approche. Il
  faudrait plutôt dire que ces approches semble intéressantes pour ton
  problème.}  paradigm called programs with effects and effect handlers as a
foundation for a compositional reasoning framework for Coq, drawn from these two
approaches \GH{quelles approches?} and which addresses our objectives
...
the second door'' then ``Open the first door''. \GH{Cette dernière partie n'est
  pas très claire. Est-ce une transition vers ce qui suit? Un positionnement de
  ton approche? Que veux tu dire par "we investigate"?}
...
system. \GH{répétition de "système"}
%
...
compositional reasoning based on assume-guarantee paradigm. \GH{et?}
%
...
\GH{il manque une transition vers la prochaine sous-section.}
...
monads\,\cite{wadler1990comprehending,jones2005io}. \GH{Dans ce qui suit, tu
  ...
  exemple de air-lock} However, monads have a reputation not to compose very
...
\GH{Idem que pour les monades. Il faudrait expliquer un peu ce que sont les
  effets algébric et les effect handlers} have been proposed to address this
...
addresses our two objectives \GH{on a un peu perdu de vue ces deux objectifs. En
  remontant un peu je m'aperçoit que tu voulais un formalisme bien établis dans
  la communauté (ok, c'est ce que tu expliques après) et d'une spec
  exécutable. Tu ne parles pas de ce deuxième point, il me semble}: we can
...
\GH{et? Le passage sur la méthode B semble un peu tomber du ciel et tu laisse le
  lecteur sur sa fin.}
...
component in isolation with a similar reasoning. \GH{on ne comprends pas ce que
  cet "additional step" apporte par rapport à l'approche initial, tel qu'il est
  formulé}
%
...
what Lanoix \emph{et al.} for the B-method\,\cite{lanoix:hal-00105041} \GH{il
  manque un verbe à cette proposition}.

\section{Conclusion}
\label{sec:sota:conclusion}
\GH{Tu as une sous-section et un section qui se suivent qui portent le même
  titre (conclusion). Ce n'est pas très élégant, il faudrait modifier un des
  titres.}

\GH{Je trouve que ta conclusion résume mal l'état de l'art. Tu te focalise trop
  sur ton approche.}
