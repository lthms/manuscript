...
with respect to functional specifications. \GH{Il faudrait ici une phrase contre-point qui disent qu'en revanche c'est moins adapté pour notre cas de figure ou les HSE peuvent être en interaction avec de multiples composants hardware et que nous nous intéressons à prouver des politique de sécurité}
%
...
flow control \GH{information flow control n'est pas une politique, c'est un mécanisme permettant de garantir une politique (intégrité et ou confidentialité), par exemple exprimée sous forme de propriété de non interference}, integrity and confidentiality\,\cite{klein2009sel4}.
%
...
would have to make significant changes prior to using it. \GH{pas d'autres limitation? Ca fait un peu léger. Peut-être que le modèle est trop spécifique aux hyperviseurs?}
...
\GH{je trouve cette conclusion assez difficile à suivre et un peu capilo tracté. Une conclusion doit être limpide et simplement synthétiser ce qui a été présenté avant. 
. Je pense qu'il faut faire plus simple avec des phrases courtes. Tu as décidé de t'intéresser à un mécanisme HSE qui illustre bien la problématique des HSE : la SMM. Les modèles existants étant peu adapté a ce mécanisme, tu as décidé d'écrire ton propre modèle, ce qui t'a également permis d'identifier des propriétés intéressantes pour établir un modèle plus général qui puisse être utilisé pour vérifier différents HSE. Concernant l'approche, il te parait intéressant d'utiliser un theorem proover pour plusieurs raisons (xx et yy) même si l'exemple présenté par la suite aurait pu être prouvé à l'aide d'autres approche. }
...
\GH{Tout comme la conclusion précédente, cette introduction est à revoir. Elle manque de clareté. Elle doit être concise}
...
met. \GH{On ne comprend pas pourquoi tu nous parles subitement de ces deux approches (et pourquoi tu n'en n'a pas parlé dans la section précédente)}
%
Compositional reasoning \GH{c'est quoi? Tu ne l'as pas défini avant} is not limited to reasoning about components in
isolation, it also allows for verifying their composition.
%
If a component \( C' \) is proven to enforce \( A \), then the composition of
\( C \) and \( C' \) enforces \( G \).

Our second contribution generalizes the key idea of our formal theory of \ac{hse} mechanisms
...
architecture model.
%
\GH{On est dans l'état de l'art. Pourquoi tu nous parles déjà de ta contribution? Il faut évoquer les problèmes, puis parler des approches de l'état de l'art qui pourrait permettre de résoudre ce problème mais en fait pas tellement. Puis évoquer ta contribution. La tu le fait dans le désordre.}Given three components, such that a component \( R \) uses a component \( S \)
...
Because \( S \) uses \( T \), such a proof cannot but depend \GH{cannot but depend?} on how \( T \)
operates.
%
...
\item we want to use well-established constructions in functional programming
  communities \GH{pourquoi? Il faut rappeler ici que Galina est un langage fonctionnel. Donc l'aspect fonctionnel t'es imposé par le choix de ton outil. Ca ne transparait pas assez!} in order to favor adoption, and
...
%
\GH{Il manque une transition avec la suite. On comprend pourquoi tu va nous parler de la composition dans les LTS et de l'algebre de processsus. Mais alors, pourquoi va tu nous parler d'une troisième chose après?}
We then explain how we take advantage of a functional programming language \GH{encore une fois, tu mélanges la description de ton approche avec l'état de l'art. "how we take advantage" laisse penser que tu décrit ton approche. Il faudrait plutôt dire que ces approches semble intéressantes pour ton problème.}
paradigm called programs with effects and effect handlers as a foundation for a
compositional reasoning framework for Coq, drawn from these two approaches \GH{quelles approches?} and which
...
the second door'' then ``Open the first door''. \GH{Cette dernière partie n'est pas très claire. Est-ce une transition vers ce qui suit? Un positionnement de ton approche? Que veux tu dire par "we investigate"?}
...
parallel. It comes with a proof system to verifying the resulting system. \GH{répétition de "système"}
%
...
monads\,\cite{wadler1990comprehending,jones2005io}. \GH{Dans ce qui suit, tu supposes que tout le monde sait ce que sont les monades. J'en doute. En outre, tu as été super pédagogique avec ce qui précède (et qui est finalement assez éloigné de ton approche) et tu vas très (trop) vite sur la fin, alors que ce sont les travaux les plus proches! Tu pourrais par exemple utiliser ton exemple de air-lock} However, monads have a reputation
not to compose very well\,\cite{hyland2006combining}, despite constructions such
as monad transformers\,\cite{liang1995mtl}.

Recently, algebraic effects and effect handlers\,\cite{bauer2015effects} \GH{Idem que pour les monades. Il faudrait expliquer un peu ce que sont les effets algébric et les effect handlers} have
...
addresses our two objectives \GH{on a un peu perdu de vue ces deux objectifs. En remontant un peu je m'aperçoit que tu voulais un formalisme bien établis dans la communauté (ok, c'est ce que tu expliques après) et d'une spec exécutable. Tu ne parles pas de ce deuxième point, il me semble}: we can leverage the ``do-notation'' ---already
...
\GH{et? Le passage sur la méthode B semble un peu tomber du ciel et tu laisse le lecteur sur sa fin.}
...
component in isolation with a similar reasoning. \GH{on ne comprends pas ce que cet "additional step" apporte par rapport à l'approche initial, tel qu'il est formulé}
%
...
what Lanoix \emph{et al.} for the B-method\,\cite{lanoix:hal-00105041} \GH{il manque un verbe à cette proposition}.

\section{Conclusion}
\label{sec:sota:conclusion}
\GH{Tu as une sous-section et un section qui se suivent qui portent le même titre (conclusion). Ce n'est pas très élégant, il faudrait modifier un des titres.}

\GH{Je trouve que ta conclusion résume mal l'état de l'art. Tu te focalise trop sur ton approche.}
