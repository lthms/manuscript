...
of Jomaa \emph{et al.}\,\cite{jomaa2016mmu}, who specified what
requirements a microkernel must satisfy in order for a \ac{mmu} to enforce memory
...
approaches that enable the ``divide and conquer'' strategy to reduce the burden
of verifying large systems (Section~\ref{section:sota:compsec}).
...
implementation.
%
Many x86 models have been proposed over the years, but to the extent of our
...
%
% The literature has a rich collection of formalisms
%%
% \GH{lequels? Tu pourrais en citer quelques uns à titre d'example, avec des ref
% pour appuyer "a riche collection of". Ou donner une ref qui parle de la
% profusion des modèles}
%%
% which can be leveraged to that end, each of them being well-suited for
% dedicated classes of problem.  \emph{e.g.}  the Kripke
% structure\,\cite{kripke1971semantical} is a prominent definition of transition
% systems used in model checking\,\cite{clarke1999model}.
%
% In practice, l
...
\begin{example}[Airlock Safety and Liveness Properties]
  ...
  \begin{proof}
    ...
    of \( R \), and after unfolding \( \eta \), we conclude using the
...
components. Verifying a \ac{hse} mechanism consists of proving that this set of
...
% \GH{Je trouve que ce tour d'horizon des méthodes formelles est un peu
% rapide. Il gagnerait à être plus complet et mieux structuré.}
%%
% \thomasrk[inline]{Suite à notre conversation téléphonique, j’ai changé mon
% fusil d’épaule. J’ai enlevé la partie sur la construction de la preuve (sans
% doute intéressante, mais pas forcément utile), et j'ai changé la mentalité de
% la présentation des différentes logiques pour en faire une présentation des
% outils dispo et pourquoi on a choisit Coq.}

Formal verification leverages two classes of approaches with their dedicated
tools.
%
On the one hand, the most generic approach is the construction of a proof, that
is a succession of inference rules to derive the statement to be proven from
...
%
On the other hand, it is possible to rely on algorithms whose correctness has
...
problems it can cover, but it also reduces the level of automation of the tools
used to verify its statements.
%
Fully automated procedures rely on algorithms whose computational complexity
impairs their applicability for more complex problems, \emph{e.g.}  model
...
respect to the applied terms, and \emph{quantifiers}, such as \( \forall \) (all
...
%
Hence, in the theory of natural numbers, an SMT solver will find that the formula
\( x > y \wedge x + y = 5 \) is satisfiable with \( \{ x = 3, y = 2 \} \), but
the formula \( x - y = 0 \wedge x \neq y \) is not satisfiable by any
assignment.
%
The lack of quantifiers has a significant impact on formula expressiveness
...
Linear Temporal Logic (LTL)\,\cite{sistla1985ltl} and Computation Tree Logic
...
%
CTL considers trees of possible futures (in opposition to a \emph{linear}
future).
%
...
infinite traces.
%
Temporal logic formulas are commonly verified using model checkers,
...
natural numbers, booleans, or more complex structures such as the states of an
airlock system).
%
...
We leverage a higher-order logic to define our formal theory of \ac{hse}
mechanisms.
%
...
hardware features, as applications typically do, and to reason about correctness
...
and the many features exposed by the latter may interfere with each other.
...
%
Another category of x86 models is ad-hoc models, especially developed to verify a
...
authors have proven this model correctly enforces that security policies
including integrity and confidentiality policies\,\cite{klein2009sel4}.
%
...
in order to reduce the assumptions made by the model for the hardware.
%
...
hypervisor--- constitutes an interesting approach that we could leverage for
modeling the interactions between the software components and the hardware
model.
%
...
against the ARM internal conformance testsuite..
%
Once the level of trustworthiness of the ASL specifications have been
established, they have been able to leverage them to formally verify properties
of the hardware architectures, \emph{e.g.} by compiling them to a subset of
Verilog accepted by commercial Verilog model checkers\,\cite{reid2016end}.
%
...
%
This represents an exciting opportunity for research targeting ARM architectures,
...
%
A \ac{xom} \ac{cpu} keeps track of each memory location owner, using a
...
model.
%
This methodology is easily expressed in a higher logic.
%
...
%
This experience convinced us of the interests of a general-purpose hardware model
...
%  %
% \item the existing models focuses on providing an abstraction of a x86
%   hardware architecture to reason about machine-code, or on a particular sets
%   of hardware features to verify them in isolation or because they are
%   leveraged by a system software component,
%  %
% \item we anticipate a great care should taken in defining our required model,
%   to manage the scale of the x86 hardware architecture.
% \end{inparaenum}
%%
% It would require a tremendous effort to be developed, reviewed and validated,
% and the question of its applicability in a verification process would have to
% be demonstrated.
%
% Therefore, we decided to compromise and experiment on an ad-hoc model with the
% aim of identifying desirable properties for a large-scale, general-purpose
% model that would optimize the applicability of our approach; and because the
% scope of existing models does not cover the features that a \ac{bios} relies
% on to enforce its isolation, we developed our own minimal model.
%%
% \GH{Je trouve cet argumentaire un peu casse-gueule... Tu laisse entendre que
% les modèles existants sont incomplet et spécifique à un mécanisme. Or tu as
...
The formal verification of a system as complex as the x86 hardware architecture
...
%
In the context of compositional reasoning, the system is broken down into
several components.
%
...
% \GH{Tout comme la conclusion précédente, cette introduction est à revoir. Elle
% manque de clareté. Elle doit être concise}
...
%
% Our approach is reminiscent of compositional reasoning approaches, such as
% \GH{On ne comprend pas pourquoi tu nous parles subitement de ces deux
% approches (et pourquoi tu n'en n'a pas parlé dans la section précédente)}
%
...
% fulfill their promises--- with compositional reasoning, in order to address
% the challenge posed by the inevitable complexity of a general-purpose hardware
% architecture model.
%%
% \GH{On est dans l'état de l'art. Pourquoi tu nous parles déjà de ta
% contribution? Il faut évoquer les problèmes, puis parler des approches de
% l'état de l'art qui pourrait permettre de résoudre ce problème mais en fait
% pas tellement. Puis évoquer ta contribution. La tu le fait dans le
% désordre.}Given three components, such that a component \( R \) uses a
...
%%
% Because \( S \) uses \( T \), such a proof depends on how \( T \) operates.
%%
% We propose to define another pair of assumption and guarantee for \( T \), and
% assume \( T \) is correct with respect to this pair.
%%
% In this context, it is possible to prove that \( S \) enforces its guarantee
% if it satisfies the assumptions of \( T \), and if the guarantee of \( T \)
% are sufficient about the guarantee of \( S \).
%%
% In practice, \( T \) may use another component \( U \) to operate, and a proof
% of correctness for \( U \) will therefore follow the same approach.
...
\begin{example}[Airlock System as Interface Automata]
  ...
  %
  A door has two states: it can either be open or closed.
  %
...
%
They allow for modeling, by means of formal languages, interacting components
as concurrent processes which exchange messages through channels.

\subsection{Process Algebra}
\label{subsec:sota:palgebra}

Process algebras and their proof systems have been developed to reason about
...
\begin{example}[Airlock System in \( \pi \)-calculus]
  ...
  \item \( P + Q \) is the nondeterministic choice operator, we use it here in
    conjunction with guards to implement an \texttt{if-then-else} construct. That
...
%
Their proofs consist of a succession of refinement proofs from their
implementation to high-level modules which model instructions semantics and
...
% GH: et?
%
% Thomas: l'intro dit maintenant clairement qu'on veut faire du
% assume-guarantee.
%
We now give an overview of compositional reasoning framework implemented in
theorem provers.

\subsection{Compositional Reasoning for Theorem Provers}
\label{subsec:sota:peff}

{\scshape Kami} demonstrates that it is possible to model components in
isolation, with an emphasis on how they interact together.
%
...
components has been less studied in the theorem proving community, with the
...
operations which update these variables. Preconditions in the form of predicates on
...
interested in.
%
However, the B-method proof environment is quite different from other theorem
provers, such as Coq.
%
In particular, B is similar to imperative languages, when {\scshape Gallina} is
a purely functional language.

To the extent of our knowledge, the \texttt{Coq.io} framework, developed and
...
---that is reasoning about functional programs in presence of side effects--- and
...
Modeling side effects in pure programming language, such as Haskell or {\scshape
  Gallina}, is usually achieved with
...
%
We believe they allow for modeling components similarly to what {\scshape Kami}
achieves, but constitute at the same time a more familiar formalism for the
