...
%
\GH{la tournure "assuming provided an operational semantics for $\J$" me semble
  bizarre mais je comprends ce que tu as voulu dire. Reformuler?}
...
\GH{la j'avoue, j'ai décroché sur la définition 6.7. Peut-être qu'un petit
  exemple ou une petite description de la formule en anglais aiderait à la
  compréhension.}
...
\begin{definition}[Abstract Specification] \label{def:freespec:abstract-specs}
  ...
  \item $\func{post}$ is the postcondition over effects results, \emph{i.e.} a
    ...
    the results $x$ computed for the effects $e$ satisfies the postcondition
...
when a component is being used by more than one components

\paragraph{Predicate of Synchronization.}
%
The most favorable scenario consists of one component which uses many
components, and is only used by one other component. \GH{fait une figure pour
  illustrer ce cas}
%
...
\begin{example}[Memory Controller Compliance]
  ...
  semantics from $C_{\mathrm{MCH}}$ which complies with
  $A_{\mathrm{MCH}}$.
  %
  \GH{lien vers le code Coq}
\end{example}

\paragraph{Predicate of Non-Interference.}
%
\GH{je trouve le terme non-interference casse gueule... Il faudrait a minima
  dire que c'est différent de la propriété de non-interference classiquemen
  utilisée en IFC... Le mieux est de trouver un autre nom}
%
...
\begin{example}[Memory Controller and IOMMU]
  ...
  \( \func{pnf}_{\mathrm{IOMMU}} \) predicate, then we can safely compose it
...
%
In other word, this verification process can help uncover inconsistencies in
...
%
\GH{on ne comprend pas trop pourquoi}
