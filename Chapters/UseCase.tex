...
previous state\PC{and exit \ac{smm}}.
%
...
flash memory.
%
This second\PC{c'est quoi le premier ?} access control mechanism is implemented by the \ac{pch}.
%
...
\item Attackers issue a memory write access targeting the SMRAM, and because of
  ...
  cache, but does not pass the access to the Memory Controller.\PC{``pass the
access'' maladroit}
%
...
%
the former\PC{latter?} is keeping copies of successful accesses to decrease latency due to
memory accesses.
%
...
\paragraph{Intel TXT.}
%
The Intel Trusted eXecution Technology (TXT)\cite{intel2015txt} is a
recent\PC{bof, on en trouve déjà en 2009: https://lwn.net/Articles/326747/}
feature of some x86 \acp{cpu}, whose purpose is to attest the integrity of the
system software component program without the need to trust the \ac{bios}.\PC{et
surtout, fournir un environnement d'exécution de confiance}
%
...
%
As a consequence, Intel was\PC{is?} advising system software developers to enable back
...
previously, such that setting \texttt{BIOSWE} was triggering\PC{ce ``was
triggering'' est bizarre``. Tu veux dire '''which triggers``?} a \ac{smi} to
