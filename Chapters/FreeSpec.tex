...
%
The Coq proof assistant\,\cite{coq} has proven to be effective to model
specific hardware
...
Reasoning about side effects in purely functional languages such as {\textsc
  Gallina}, the Coq specification language, is difficult, firstly
because they imply to somehow take into account an outer \emph{stateful}
environment and secondly, because the composition of stateful computations is
not well-handled by traditional (monadic) approaches.
%
...
\item We have implemented FreeSpec\,\footnote{FreeSpec has been released as a
    ...
    \url{https://github.com/ANSSI-FR/FreeSpec}}, a framework for the Coq
  proof assistant to \emph{modularly} verify programs with effects and effects
...
\paragraph{Model Modularity}
%
Here we talk about the scale of the model.

\subsection{Approach Overview}

\begin{figure}
  \begin{center}
    \subfloat[Verification of the CPU]{
      \begin{tikzpicture}
        % styling nodes
        % -------------------------------------------------------------------
        [baseline, thick, scale=0.8, every node/.style={transform shape},
        component/.style={draw, rectangle,inner sep=5pt, text badly centered},
        interface/.style={font=\footnotesize}]
        % --------------------------------------------------------------------------------

        % defining nodes
        % -----------------------------------------------------------------
        \node (S) [component, dashed] {Software};%
        \node (CPU) [component, right=of S] {CPU};%
        \node (MC) [component, right=of CPU, dashed] {Memory Controller};%
        % --------------------------------------------------------------------------------

        % defining edges
        % -----------------------------------------------------------------
        \draw [-latex] (S) to node [interface, above] {$I_{\text{CPU}}$} (CPU);%
        \draw [-latex] (CPU) to node [interface, above] {$I_{\text{MC}}$} (MC);%
        % --------------------------------------------------------------------------------
      \end{tikzpicture}
    } \quad \subfloat[Verification of the Memory Controller]{
      \begin{tikzpicture}
        % styling nodes
        % -------------------------------------------------------------------
        [baseline, thick, scale=0.8, every node/.style={transform shape},
        ...
        % defining nodes
        % -----------------------------------------------------------------
        \node (CPU) [component, right=of S, dashed] {CPU};%
        \node (MC) [component, right=of CPU] {Memory Controller};%
        \node (DRAM) [component, above=of MC, dashed] {DRAM Controller};%
        \node (VGA) [component, below=of MC, dashed] {VGA Controller};
        % --------------------------------------------------------------------------------

        % defining edges
        % -----------------------------------------------------------------
        \draw [-latex] (CPU) to node [interface, above] {$I_{\text{MC}}$} (MC);%
        \draw[-latex] (MC) to (DRAM);%
        \draw[-latex] (MC) to (VGA);
        % --------------------------------------------------------------------------------
      \end{tikzpicture}
    }
  \end{center}
...
\[
  \mathtt{HANDLER}_I \times \Omega \rightarrow A \times \mathtt{HANDLER}_I \times
  \Omega
\]
...
Modelling programs with side effects in purely functional languages such as {\textsc
  Gallina} (the Coq specification language) or Haskell is usually achieved
thanks to monads\,\cite{hoareetal2001monad}.
%
...
\begin{definition}[Predicate of Non-Interference]
  ...
  $\func{pnf} : \I \rightarrow \ac{prop}$ be a subset of
  effects. $\func{pnf}$ is said to be a predicate of non-interference of $A$
  (denoted $A \| \func{pnf}$) if and only if \[
    \begin{array}{l}
