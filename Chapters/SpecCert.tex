...
policies as predicates on sets of traces (\ref{subsec:speccert:def}).
%
...
\begin{definition}[Safety Property]
  ...
  property $P$.
  %
  A set of trace $R$ satisfies $\partial^2 P$ if and only if every transition of
  every trace $\rho \in R$ satisfies $P$, that is
  %
  \[
...
\chapter{Reasoning about Software Stack Isolation}
...
\begin{definition}[Partial Execution Isolation Policies]
  ...
  software components, we write $I|U$ for the partial execution isolation policy
  whose purpose is to enforce that all components of $U$ are constrained with
  respect to $I$, that is
  %
  \[
    I|U(h, l, h') \triangleq \forall u \in U, \forall x \in S, h \xrightarrow[u
    \leadsto x]{l} h' \Rightarrow u \ge x
  \]
  %
...
\section{Specifying and Verifying an Idealized BIOS HSE Mechanism}
...
\subsection{{\scshape Minx86}: a Minimal x86 Hardware Model}
...
\subsection{Specifying and Verifying a BIOS HSE Mechanism}
