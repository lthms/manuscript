...
classes of security policies described in Section~\ref{subsec:state:secu}
(Section~\ref{sec:speccert:security}).
%
Finally, we give a formal definition of HSE mechanisms and HSE mechanisms
...
%
We then give a generic definition of runs of a \ac{lts}, so we can later reason
about security policies and HSE mechanisms as properties on runs
(\ref{subsec:speccert:run}).
...
%
We propose a definition for these runs, and we later use this definition to
reason about security policies and HSE mechanisms as properties on runs.
%
We want to emphasize that our approach remains suitable to use with model
checkers, as long as their definitions of transitions and runs is equivalent
with ours.

\begin{definition}[Runs]
  We write $\pathesLTS{Ez}$ for the set of runs of the Computing Platform
  ...
  $\func{init}(\rho) : \pathesLTS{Ez} \rightarrow H$ maps a run of $\Sigma$ with
  ...
  the function which maps a run of $\Sigma$ to the set of transitions which
  occur during this run, such that
  %
  \[
...
We now discuss how we can model the different security policies discussed in
