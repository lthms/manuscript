...
abstraction layers, from the hardware components  to
...
\item [Firmware/BIOS]
  ...
  %
  \TODO{Il faudrait une petite phrase de liaison ici du style 'Hardware manufacturers tend to embed some features in their firmware for several reasons. First... }
  ...
  its processors in 2008. \TODO{Ajouter une ref}
  %
  ...
  initialization of the computer. \TODO{Phrase bien alambiqué. En outre, éviter les formulation floue du style "almost always" Du coup, préférer une formulation plus neutre du style "Firmware is also used to initialized hardware components. This initilization consists in... In particular, some firmware is responsible for the initialization of the platform as a whole.}
  %
  Afterwards, when we refer to this specific type of firmware, we use the term
  \ac{bios}. \TODO{Donner la définition de BIOS (que veulent dire les acronyme) et dire qu'aujourd'hui la plupart des BIOS respecte le standard UEFI.}
  %
\item [System Software]
  ...
  %
  \TODO{Attention, software, hardware et firmware sont normalement indénombrable en anglais. Il faut que tu identifies les manières idiomatique d'utiliser ces termes }A system software leverages this environment to provide high-level interfaces
...
%
We have implemented our proofs of concepts in the \index{Coq} proof assistant.
%
Our choice is motivated by the following reasons:
%
\begin{itemize}
\item {\sc Gallina}, the specification of \index{Coq}, is an expressive formal language,
  that combines both a higher-order logic and à powerful dependent type system,
  while {\sc Ltac}, the proof tactic language of \index{Coq}, enables powerful
  automation features.
  %
  This suits well for specifying security properties to be enforced by HSE
  mechanisms, as they can take many forms.
\item The extraction mechanism of \index{Coq} allows for transpiling formal
...
%
We have implemented SpecCert, a framework for the \index{Coq} proof assistant
...
alike.
%
In addition, we have developed FreeSpec, a framework for the \index{Coq} proof
assistant.
%
