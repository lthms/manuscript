...
%
\TODO{Ajouter une phrase d'intro du style "Vulnerabilities in HSE are often due to some misuse by trusted software" ou un truc du genre.}
...
Architectural attacks result in \TODO{are due to (c'est plutôt une cause qu'une conséquence)} a flaw in the specifications of the computing
platform.
%
...
the previous proofs hold, meaning this feature does not introduce an \TODO{any?}
...
scattered across many documents. \TODO{En outre, cela à le mérite d'établir une liste exauhstive et minimaliste des exigences à respecter pour espérer obtenir les garanti de sécurité promise. La documentation, outre le fait qu'elle est éparpillée, est rarement aussi claire à ce sujet.}
...
%
Because architectural attacks often come from unsuspected places \TODO{Donner un exemple, par exemple le cache, qui n'est pas un mécanisme de sécurité}, it is
necessary to consider the whole computing platform. At the same time, many
components are individually complex. \TODO{Donner un exemple avec des données qualitative/quantitative.}
%
...
similar role. \TODO{Détail un peu tes idées. Ne fais pas trop d'implicite. De nouveaux mécanismes hardware -> refaire la preuve. Le même mécanisme pour différente plateforme (exemple?) ->  conséquence?}
%
...
thesis. \TODO{euh... non je ne formulerais pas çà comme çà! :-)}
...
abstraction \TODO{lower lever of abstratcion? low-level components?} grows in importance.
%
...
\item defining hardware models which can remain exploitable in presence of large
  architecture hardware. \TODO{C'est peut-être s'avancer un peu que de prétendre qu'on vise le passage à l'échelle. On s'attaque surtout à la modularité qui permettra peut-être de s'attaquer à des modèles complexes (mais c'est indirect)}
\end{inparaenum}
...
trusted software components execution from the rest of the software stack. \TODO{Phrase longe, couper en deux? "to specify and verify HSE mechanisms against a hardware Model. The objectif of such HSE is ...}
%
...
x86 model, and we have verified the HSE mechanism implemented by x86 BIOSes.\TODO{La description du HSE est un peu vague. il faudrait dire que c'est le HSE pour protéger la SMM. En outre, tu as vérifié ce HSE sur Minx86 (il faut le dire explicitement)}
%
...
verify large \TODO{encore une fois, "large" est peut-être de trop... tu n'as pas montré que ton approche passe l'échelle. Par contre, le côté modulaire, pas de soucis. }systems, organized as trees of components, in a modular way.
%
