...
%
Theorem provers such as Coq\,\cite{coq} or HOL/Isabelle\thomasrk{cite} provide
...
%
Abstract interpretation \thomasrk{cite} and model checking \thomasrk{cite} are
two notorious instances of this approach.
...
The most common formal system is the first-order logic\thomasrk{cite},
characterized by:
%
\begin{itemize}
...
%
Higher-order logic \thomasrk{ref} does not suffer the same limitation, so it
becomes possible to express statements such as \emph{for all sets with a total
  order \( < \), for all pair of values \( \alpha, \beta \), then either
...
%
On the one hand, \emph{Interactive} theorem provers (\emph{e.g.}
Coq\,\cite{coq}, HOL/Isabelle\thomasrk{ref}, or more recently
Lean\thomasrk{ref}) are based on higher-logic.
%
\emph{Automated} theorem provers (\emph{e.g.} Vampire\thomasrk{ref},
Z3\thomasrk{ref}) are based on first-order logic.

Finally, modal logic \thomasrk{ref} is a formal system which extends first-order
logic with modal operators.
%
Temporal logics form the most popular family of modal logics, such as Temporal
Linear Logic (LTL)\thomasrk{ref}.
%
...
%
Another popular temporal logic is Computation Logic Tree (CTL)\thomasrk{ref},
which considers trees of possible futures (by opposition to a \emph{linear}
future).
%
...
\emph{e.g.}  NuSMV\thomasrk{ref}, SPIN\thomasrk{ref}, or TLA+\thomasrk{ref}.
%
