...
\PC{pas que l'implémentation - souvent c'est le modèle uniquement}
%
To that end, a verifier \PC{c'est qui/quoi en pratique ?}
%
\begin{inparaenum}[(1)]
...
\end{inparaenum}
%
\PC{saut de page --- de manière générale, il faut casser les paragraphes
``massifs'' qui mélangent des idées, et ne sont pas plus simples à lire}
...
another.
\PC{c'est curieux d'écrire que tout est variable, alors que que tu viens d'en
	donner une définition. Assure toi de donner une définition
standard et reconnue de ces termes}
%
...
%
Finally, we describe how the concepts of this section have been used\PC{par
  qui?} in order to verify hardware and software systems
...
\begin{example}[Airlock System]
  ...
  \item A door of the system can be either \( \mathtt{open} \) or
    \( \mathtt{close} \).
    %
    The set of states of the airlock system reflects the combinatronics\PC{???}
    of the doors states.
...
systems.
%
For instance, the Kripke structure\,\cite{kripke1971semantical} is a
notorious\PC{pas notorious (négatif). ``classical'' ?}  variation of (labeled)
transition systems used in model checking\,\cite{clarke1999model}, while Petri
net\,\cite{peterson1981petri}, process algebra\,\cite{bergstra1984process} or
interface automata\,\cite{de2001interface} are better suited to model concurrent
systems.
