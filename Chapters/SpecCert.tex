...
\end{inparaenum}
%
As a consequence \TODO{je ne vois pas trop le lien de cause à effet. Revoir la transition de cette phrase}, we have broken down our formal
definition into three components: hardware models, \ac{hse} mechanisms and
security policies. \TODO{Attention à l'usage des temps. Dans cette phrase tu utilises le present perfect. Ensuite c'est le présent...}
%
First, we model the hardware architecture as a labeled transition system whose
sequences of transitions, also called traces, characterize every possible execution of 
software components  (Section~\ref{sec:speccert:hardware}).
%
...
that we present throughout this Chapter. We have used these proofs to phrase
the proof sketches which follow. \TODO{Je ne suis pas sur de comprendre précisemment la dernière phrase.}

\section{Hardware Model}
\label{sec:speccert:hardware}

We model a hardware architecture, which can execute different software components, as a
\ac{lts}.
...
consecutive transitions, the resulting state of the first one is the initial state
of the next one. \TODO{Tu ne donnes qu'une définition informelle (en anglais) des traces. Tu pourrais donner la formule correspondante. Idem pour init et trans}
...
As we detailed in Subsection~\ref{subsec:state:secu} \TODO{le label de cette sous-section n'est pas valide}, security policies are
...
wherein the requirements over state are not satisfied. \TODO{Tu devrais dès le début bien insister sur le fait que ces contraintes ne s'applique que sur le code trusté. On va supposer que le code trusté utiliser correctement le mécanisme hardware. Dans notre modèle d'attaquant, l'attaquant n'a que le controle du code non trusté, qui n'est donc pas contraint.}
%
...
mechanism. \TODO{Peut être rappeller que "correctly implement the HSE" veut dire respecter les contraintes sur les états et les transititions software. C'est un peu trop implicite.}
%
...
\paragraph{Definition.} \TODO{Le nom du paragraphe (Definition) n'est pas top. Il y en a plein partout, des définitions. Je propose quelque chose du style "HSE mecanisms"...}
%
...
transitions.
%
Indeed, these requirements can only concern trusted software transitions. \TODO{Cela devrait être expliqué un peu plus en détail et plus tôt. Il suffirait alors ici de le rappeler (comme expliqué en section xx, ces contraintes ne concernent que les transition trustés.}
%

Dealing with multi-core architectures is possible, but requires additional
efforts. \TODO{J'ai l'impression que cela devrait être mentionné et discuté plus haut. De manière générale, comment modéliser une archi multi-core? Plusieurs transitions peuvent-elles arriver simultanément?}
%
One solution is to define a set of core identifiers to act as an additional
parameter for hardware-software mapping and software transitions. \TODO{Pourquoi seulement sur les transitions software? Si un coeur fait un défaut de page, est-ce que l'interruption est transmise à tous les coeurs?}
%
However, this is out of the scope of this Chapter. \TODO{C'est du bottage en touche peu élégant. Je dirais que c'est une limite du modèle tel qu'il est présenté dans cette thèse. Mais on pourrait étendre le modèle pour traiter le multi-coeur. Il faut le dire clairement. C'est pas un truc hors sujet du chapitre (sauf erreur de ma part, le problème n'est pas traité ailleurs dans le manuscrit). }
...
\begin{definition}[HSE Mechanism]
  ...
  \item $\func{software\_req}$ is a predicate on $H \times L_S$ to distinguish
    between safe software transitions and potentially harmful ones. \TODO{Je rajouterai une phrase pour enfoncer le clou du style "ce prédicat ne contraint que les transition trusté"}
  \end{itemize}
\end{definition}
...
%
As a reminder, such mechanism enforces the integrity of
the \ac{bios} code within the flash memory and is implemented by the \ac{pch}.
%
...
memory.
%
as follows: \TODO{Il manque le début de la phrase.}
%
\begin{enumerate}
...
\item In order to protect the integrity of the flash memory content, the
  ...
  execution of the system software with the \texttt{rsm} assembly instruction. \TODO{Avant de faire un lock, je suppose que le code du firmware fait quelque chose d'utile (mettre à jour le firmware?) Il faudrait le préciser.}
\end{enumerate}
...
\TODO{Dans le schéma, il faudrait agrandir le bloc "Flash memory" pour que "memory tienne sur une seule ligne"}

\begin{example}[Flash Memory Lock-down Mechanism]
  \label{example:speccert:flashdef}
  In this example, the trusted software component ($T$) is the \ac{bios}. \TODO{ et du coup, il faudrait préseciser que les autres composants (OS, applications, etc.) sont les non-trustés.}
  %
  The \ac{cpu} executes the \ac{bios} when it is in \ac{smm} (\func{context}). \TODO{La phrase n'est pas suffisamment préciser. En outre, j'ai mis du temps à me rappeler que context ne doit pas être pris ici comme un mot commun mais un nom de fonction!. Il faudrait dire que par défaut contexte(h)=non trusté sauf lorsque le CPU est en SMM, c'est à dire lorsqu'une SMI se produit et avant que le bios exécute une instruction rsm}
  %
...
%
For a \ac{hse} mechanism to be correctly defined \TODO{est-ce que "consistent" ne serait pas mieux que "correct", dans ce cas? Les lois sont la pour assurer la cohérence de la définition.}, it must also obey a few \TODO{a few est un peu vague. Two?"}
...
\TODO{Pour être cohérent entre les deux définition, j'utiliserait plutôt un "ET" logique plutôte qu'une imbrication d'implication. Si hardware\_req(h) ET  l in LS...}
...
policy. \TODO{Cela veut surrement dire que les contraintes sur l'exécution du code trusté sont incomplètes ou que le la restriction sur l'état du hardware est au contraire trop restrictive. S'il n'est pas possible de trouver une contrainte sur l'état du hardware moins restrictive permettant d'assurer la propriété de sécurité ou de restreindre le code trusté pour vérifier cette seconde loi, il y a de forte chance que le HSE ne permette pas d'assurer la propriété de sécurité visée.... }
...
\begin{example}[Flash Memory Lock-down Inconsistency]
  ...
  Law, but \emph{does not} obey the first one. \TODO{Euh, sauf erreur de ma part, c'est l'inverse. Il obeit à la première mais pas à la deuxième.}
  %
...
easier to reason about, as shown by the following theorem. \TODO{Je trouve que c'est un peu s'avancer de dire que ton modèle peut traiter autre chose que des prédicats sur les transitions. Tout est construit dans ton approche pour traiter efficacement ce cas. Tu ne nous convainc pas que ton approhce (prédicat, sur l'état du hard, restriction sur les transition trusté...) est adaptée pour traiter d'autre cas...}
...
\TODO{Contrairement au lemmes précédents, je trouve que la preuve (schéma de preuve?) est un peu trop rapide. Je n'arrive pas à suivre et à me convaincre que çà marche (mais je suis sans doute trop fatigué).}
...
\TODO{Sauf erreur de ma part, ton chapitre 4 ne développe pas du tout cet aspect "modulaire" et "réutilisabilié". Donc c'est un peu exagéré de le mettre en avant dans la conclusion. Soit il faut nuancer cette conclusion, soit il faut développer ces aspects (via un exemple notamment) dans le chapitre.}
