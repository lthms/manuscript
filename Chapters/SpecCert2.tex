...
\TODO{es-tu vraiment le seul (et le premier) a parler de isolation policy? c'est
  le "we call" qui me fait tiquer}.
%
Such a policy prevents a software component to tamper with the execution of
lower levels of the software stack. \TODO{Pourquoi s'embarquer dans les histoire
  de position dans la software stack. J'ai pas l'impression que cette histoire
  de hierarchie dans une stack est été formellement définie. Je préfererais
  qu'on parle de code trusté/non trusté.}
%
...
former achieves a code injection against the latter. \TODO{Evite les longues
  phrases avec "le premier, le second, etc.". A la fin on s'y perd. Il est
  préférable dans ce cas de nommer les deux composants et d'utiliser leur nom
  par la suite. Par exemple, soit deux composant A et B... } \TODO{Outre la
  remarque précédente, je n'ai pas l'impression que "make the latter execute an
  instruction of its choice" soit synonyme de "code injection". J'ai
  l'impression que la définition inclut également du Code Reuse Attack
  (l'attaquant fait bien exécuter du code de son choix à la cible également dans
  ce cas, même si les instruction appartiennent à du code légitime de la
  cible). En outre, si c'est bien de "code injection" dont tu veux parler, dit
  le dès le début et n'essaie pas de redéfinir ce concept d'une manière
  alambiqué et au final ambigue.}
%
This is a particular sub-case of the Integrity property
(Definition~\ref{def:usecase:int}),
%
\TODO{Le label de la définition est cassé}
%
as we focus on instructions fetches exclusively.
%
\TODO{Pas claire mais je ne suis pas allez chercher ce que la propriété
  disait..}
%
...
%
\TODO{Cette partie justificative est un peu casse gueule, je trouve. Déjà,
  utiliser un mot qui, généralement, à un autre sens dans la plupart des travaux
  (ce que tu sembles avancer) n'est pas une bonne idée. Il faut peut être
  changer le nom, dans ce cas. En outre, je ne vois pas pourquoi une propriété
  plus forte devrait être mise en oeuvre seulement dans des couches supérieures
  (system software). A mon avis, la propriété dépend du modèle d'attaquant
  considéré. Il faudrait un peu plus développer ces aspects. Par contre, pas sur
  qu'on soit si générique dans ce cas...}
...
%
\TODO{Que veux dire "complete software stacks"?}  Finally, we introduce
...
overwrites its content during a transition. \TODO{Sauf erreur de ma part, cette
  notion de "owner" n'est définit nulle part formellement. elle devrait l'être}
%
...
\TODO{Tu définis formellement le prototype de cette fonction mais pas vraiement
  la fonction elle même. Tu devrais le faire à partir de la définition de owner
  (et possiblement de la valeur du compteur programme). En outre, sauf erreur de
  ma part, l'exécution d'une instruction se traduit pas plusieurs transitions
  software. Du coup, le mapping renvoie la même liste pour chaque transition
  correspondant à l'exécution d'une instruction ou bien seulement pour la
  première? Il faudrait clarifier tout çà (une vraie définition formelle
  aiderait...)}
...
\begin{example}[Application Code Injection]
  ...
  instructions which are owned by the OS. \TODO{Tu devrais noter avant (en
    dehors de l'exemple) que l'injection de code peut être légitime. cet example
    le montre.}
\end{example}

\TODO{tu devrais placer un peu de texte entre les exemples, notamment pour les
  introduire et faire des transitions entre les exemples}
...
\begin{example}[SMRAM Cache Poisoning]
  ...
  the \texttt{SMBASE} register. \TODO{la description de cet exemple n'est pas
    très claire. Il faudrait mieux détailler l'injection et renvoyer
    éventuellement à une section décrivant l'attaque pkus en détail si c'est le
    cas. En outre, le but final n'est pas très claire et très spécifique
    (contrairement à "escalade de privilèges" de l'exemple précédent, que tout
    le monde comprend.}
\end{example}
...
authorized to tamper with the execution \TODO{est-ce que "tamper with the
  execution" est synonyme de "make a code injection"? Dans ce cas, j'aimerais
  que tu n'utilises qu'un seul termes, et de préférence le plus précis. Tamper
  with laisse imagine d'autres formes d'interférences que l'on pourrait très
  légitimement vouloir interdir mais qui ne semble pas couverte par ton
  approche.} of another software component in the context of a software stack.
...
% \begin{corollary}
%   If a transition satisfies $I$, then no code injection which is not
%   authorized by $\rightarrowtail$ occurs during that transition, that is
%  %
%   \[
...
%   \]
%
%   \TODO{Dans ce corolaire, tu utilises la notation $ y \not\ge x$ alors
%   qu'avant tu sembles utiliser la notation $x \rightarrowtail y$ pour la même
%   chose. Soit j'ai loupé quelque chose soit il faut homogénéiser tes
%   notations. En outre, ce corollaire est-il vraiment utile ici. Il semble
%   trivial et on n'en voit pas trop l'intéret.}
%
%  \begin{proof}
...
\begin{example}[Software Stack Isolation]
  ...
  The definition of the corresponding $\func{context}$ shall obey the
  following logic: when the \ac{cpu} is in \ac{smm}, the \ac{bios} is executed;
  when the \ac{cpu} is not in \ac{smm} and is in ring 0, the system software
  component is executed; when the \ac{cpu} is not in \ac{smm} and is in ring 3,
  one of the applications ---identified by the page tables used by the
  \ac{cpu}--- is executed.
\end{example}
...
\TODO{Cette sous-section est vraiment très (trop) courte par rapport aux
  précédentes. Faut-il vraiment une sous-section dédiée?}
...
%
For instance, we use this approach to define the \ac{bios} isolation policy we
seek to define in this Section. \TODO{"to define... we seek to define" fait une
  peu répétition}
...
%  %
%   For instance, the \ac{bios} executed while the \ac{cpu} is in \ac{smm} has
%   to be careful not to tamper with the page tables previously configured by
...
to formally specify and verify a HSE mechanism, and thus is not exhaustive. \TODO{Le lien de cause à effet entre ces deux phrases n'est pas évident. En outre "a HSE mechanism" est un peu vague. En l'état, ton modèle a été défini pour pouvoir spécifier et vérifier le HSE lié à la SMM. Il vaut mieux être précis dès le début.}
%
...
System Management Interrupt (SMI), it halts its current execution and reconfigures itself to a
specified state. \TODO{"specified state" est vague. Specifié par qui?}Then it executes the code stored in memory at the address
$\mathtt{SMBASE} + \val{0x8000}$.
%
In practice, the SMBASE value points to the beginning of a memory region called the
SMRAM.
%
...
%
Among them, the \ac{smrr} are used to tell the \ac{cpu} where the SMRAM is
and which cache strategy to use for \IO targeting the SMRAM when the \ac{cpu}
is in \ac{smm}.  When it is not in \ac{smm}, the \ac{cpu} always uses the UC strategy
for \IO targeting the SMRAM. Such registers can only be
configured when the \ac{cpu} is in \ac{smm}.
%
\ac{smrr} have been introduced as a countermeasure of \TODO{to?} the SMRAM cache poisoning
attack\,\cite{wojtczuk2009smram,duflot2009smram} which allowed some untrusted code
...
%
The memory controller uses a special range of physical addresses to host the SMRAM.
%
...
\TODO{La nuance entre "written back to the memory" et "content of a memory location is overwritten during a transition " n'est pas très claire. Dans le cas d'un write back, par définition le contenu de la mémoire est modifiés...}
...
\TODO{Suite à notre discussion téléphonique, je comprends que tu n'as pas pu définir avant précisemment context car celui-ci dépend du raffinement de ton approche générique. Mais ici tu devrais pouvoir le définir précisemment (en t'appuyant sur les trois règles citées précédemment. Hors tu ne le fais pas...}
...
\TODO{Pourquoi Cache et Mem sont paramétré par S et pas les autres? Tu devrais l'expliquer brièvement ici}
...
%
By convenience, we give the same maximum address to each of \TODO{"of" n'est-il pas de trop?} address space.
...
\]
%
\TODO{Comme j'ai du le mentionner auparavant, le style de la macro func n'est pas suffisamment discernable du texte descriptif. Comme tu utilises souvent des noms communs pour tes fonctions, çà n'aide pas à bien identifier ces noms commes des noms de fonctions dans les descriptions qui suivent.}
The boolean $\func{in\_smm}$ is  set to $\val{true}$ when the \ac{cpu}
is in \ac{smm} and to $\val{false}$ otherwise.
%
...
%
The set of states of the \acp{smrr} \TODO{SMRR est noté ici en majuscule mais la variable censé les modélisé est en minuscule.} is denoted $\texttt{Smrr}$.
%
\[
...
\TODO{Dans la formule précédente, à quoi correspond "c"? Cache? Est-ce un problème d'incohérence des notations?}
...
\TODO{Pourquoi mc.dopen doit valoir "true" pour que la translation d'adresse renvoie vers le VGA? Ca me semble contre-intuitif.}
...
and hardware transitions (denoted by $L_H$). For each transition label we define
%
\begin{inparaenum}[(1)]
...
formally defined in Figure~\ref{fig:speccert2:softprepost}. \TODO{Il faut commenter cette figure (et donc, ces définitions formelles)}
%
...
to review their definitions. \TODO{Je trouve cette dernière phrase un peu maladroite. En outre, le code Coq ne fait pas parti de la thèse. Peut-être cela serait-il pertinent de mettre toutes les définitions formelles en annexes sous forme textuelle et rappeler que ces définitions sont également disponibles dans le code Coq (dans tel fichier). En tout cas, donner seulement un exemple de définition formelle dans le coeur du manuscrit me semble la bonne stratégie. Par contre, renvoyer le lecteur au code Coq pour les autres me semble un peu limite}
...
A software component can always read and write at any physical address. \TODO{Cette phrase est un peu ambigu. On pourrait comprendre que n'importe quel softare peut lire n'importe quelle adresse physique. En pratique ce n'est pas le cas car un mécanisme de mémoire virtuelle, utilisant la pagination et ou la segmentation, va s'interposer et empêcher la lecture de certaines adresses physique. Par contre, une fois cette traduction effectuée, rien n'empêche la lecture d'une adresse physique (le MC?). Il faudrait le rappeler ici.}
%
...
updated according to the update of the memory and cache states. \TODO{En l'absence de plus de détail (formalisme), difficile de comprendre précisemment cette dernière phrase}
%
...
%
This mecanism is modelled by the \func{dispatch} function.
%
...
mapping passed as a parameter of {\scshape Minx86}. \TODO{Idem remarque précédente. Difiicile de comprendre ce que tu veux dire sans le modèle explicite sous les yeux.}
...
unset. \TODO{d\_lock bit unset c'est la même chose que SMRAMC unlocked? Il faudrait éviter de décrire les même conditions avec des termes différents, çà pert le lecteur non averti}
%
...
\]
%
\TODO{Il y a simplement implication ou équivalence?}
We distinguish between \func{Read} and \func{Fetch} because a software component
does not always decide the value of the program counter register of the Core. \TODO{Je comprends la fin de la phrase mais je ne vois pas le lien avec "We distinguish between \func{Read} and \func{Fetch}". Difficile de comprendre ce que tu voulais dire.}
%
...
which own an instruction fetched during this transition. \TODO{C'est le cas de tous les transition-software mapping, non? Qu'est-ce qui est spécifique à la version Minx86?}
%
...
Figure~\ref{fig:speccert2:fetchedformal}. \TODO{Un peu rapide. Il faudrait commenter un peu cette définition. En outre, figure est-il un environnement adéquat pour ce genre de définition? (on s'attend à une figure, pas des définitions logiques...). Peut-être faudrait-il définir un environnement adéquat.}
...
%
Our objective is to give a formal definition for \TODO{for -> of?} the \ac{hse} mechanism
...
\ac{cpu}, and the \texttt{SMRAMC} register of the memory controller. \TODO{Il manque une transition avec la suite. Une annonce du plan "local", notamment.}
...
identified six requirements on states. \TODO{Cette phrase me paraît un peu étrange. -> "We have identified six req for h to satishy harware\_req?"}
%
\begin{enumerate}
\item When the \ac{cpu} executes the \ac{smm} code, the program counter register
  ...
  \]
  \TODO{Contrairement aux autres, cette propriété n'est vraie que lorsque le CPU est en SMM. Il faudrait le faire apparaitre dans la formule}
  %
...
\TODO{A la lecture de ces requirements, je m'aperçois que certains d'entre eux s'apppuient sur une configuration réalisée au préalable par le code trusté, durant la phase de boot. On peut donc dire que les exigences sur le code trusté ne sont pas seulement dans les software requirements mais aussi, pour certaines, dans les hardware requirements (et du coup le nom n'est peut être pas idéal), même si elles ne correspondent pas à la même phase d'utilisation (initialisation vs. runtime en quelque sorte)}
...
satisfies the two \ac{hse} Laws \TODO{Attention, pas de majuscules à Laws (c'est un mot commun et auparavant tu n'utilisais pas de majuscule). Fais attention, tu as tendances à utiliser les majuscules comme emphase. Fais une vérification sur tout le document pour homogénéiser}.

The first law states that the \( \func{bios\_req} \) is always satisfied when a non-trusted
software component, i.e. different from the \ac{bios}, is executed by the hardware
architecture.
%
...
\func{UpdateSmrr}. \TODO{Il faudrait conclure explicitement en affirmant que la première loi est bien vérifiée.}

The second law states the state requirements are invariant with respect to the
software requirements. \TODO{"states the state" n'est pas très heureux... }
%
...
\TODO{Je comprends ta volonté de résumer une preuve surement un peu rébarbative à décrire et surtout de ne pas réécrire ce que tu as fais en Coq. Toutefois, sans la lecture du code Coq, difficile de comprendre et d'apprécier véritablement ton travail. C'est à mon avis un problème. Ton manuscript devrait expliquer plus précisément ce que tu as fais en Coq pour qu'un lecteur averti puisse juger ton travail sans aller lire ton code. Il faudrait demander l'avis à des spécialistes (Yann?)}
...
From the security policy perspective, only the \func{Fetch} label is relevant. \TODO{Je ne comprends pas cette phrase. Il y a trop d'implicite. Il faut que tu développes un peu ton raisonnement.}
%
Unfolding the definitions previously introduced in this Chapter and its
predecessor \TODO{Arrête d'utiliser des qualificatifs relatifs pour désigner les choses comme "previously introduced" ou "its predecessor". Il y a beaucoup de concepts (définition, théorèmes, etc.) dans ton manuscrit donc à la fin on est perdu! Numérote systématiquement les concepts (déf, etc.) et nomme les explicitement "Unfolding definitions xxx".} leaves us to prove that \TODO{Je n'arrive pas à suivre. Je ne comprend pas pourquoi prouver la correctness se réduit à prouver la formule pour fetch seulement. Il faudrait prendre le temps d'expliquer.}
%
\[
  \begin{array}{l}
...
constrains the ownership of memory location related to the SMRAM. \TODO{Une fois de plus, la présentation de la preuve est un peu rapide et difficile à comprendre sans plus d'élément.}
...
\TODO{Je n'aime pas trop le "perspective". On aurait tendance à penser que c'est du future work. A la lecture de ce qui suit, c'est un peu plus qu'une simple perspective.}
...
is the complete software stack. \TODO{Je pense qu'il faudrait rappeler ici brièvement le concept : un ensemble de composants logiciels qui sont ordonnés par une relation d'ordre partiel (forme un treillis?) et I est la politique d'isolation globale.}
%
...
\TODO{Il faudrait développer un peu la transition entre le paragraphe précédent et ce paragraphe. On doit implémenter plusieurs HSE "concurrently", OK. Et du coup, quels sont les problèmes que cela pose et qui justifient la suite?}We first introduce a \ac{hse} mechanism composition operator \( \sqcap \)
...
it is out of the scope of this Section. \TODO{Je n'aime pas trop ce genre de phrase. Encore une fois "out of the scope of this section" laisse à penser que cela sera traité ailleurs dans le manuscript, mais ce n'est pas le cas. }
...
\TODO{Même remarques que précédemment, sans le code Coq, difficile de suivre et surtout d'évaluer ce qui a vraiment été fait. Il faudrait au moins mentionner que cette preuve a été réalisée en Coq (dans quel fichier, ligne, etc.}
...
defined and verified \TODO{A quoi renvoie "previously defined and verified"? Stop aux qualificatif relatif, nomme explicitement les choses!}, the \ac{bios} participates in enforcing \( I \).
%
...
execution of applications \TODO{attention, je comprends en lisant la suite que "applications" à un sens bien précis ici. Tu supposes que ta software stack est de la forme BIOS>OS>Appk. Il faudrait le rappeler (ou l'énoncer) juste avant.} with respect to \( I \) \TODO{que veut dire précisément "constrain the execution of app with respect to I'?} verifies this property.
%
We formally specify this additional security policy \TODO{plutôt que additionnal secutiy property, il faudrait dire explicitement que c'est la propriété mise en oeuvre par ce HSE complémentaire.}
%
\[
...
\begin{proof}
  ...
  $I|U_{\mathtt{os}}$ and $I|U_{\mathtt{bios}}$. \TODO{Que désignent $I|U_{\mathtt{os}}$ and $I|U_{\mathtt{bios}}$? Tu l'as peut-être définis avant, mais c'est loin. IL y a de forte chance que le relecteur soit perdu comme moi. Il faudrait faire un petit rappel et mettre une référence.}
  %
  This is enough to conclude, based on the definitions of global and partial
  execution isolation policies. \TODO{Que sont les "global et partial execution isolation policies"?}
\end{proof}
...
separately. 
\TODO{Cette dernière phrase est un peu maladroite. Globalement, cette section est mal située, je trouve. Finalement, nous n'avons pas besoin de la définition préciser de  $\Delta_{\mathtt{bios}}$ pour comprendre ces problématiques de composition. En revanche, tu fais souvent référence à des concepts définis en section 5.1 ou avant... Les idées sont intéressantes mais il faudrait mieux les vendre.}
...
architecture model to a real world example. \TODO{Phrase trop longue et difficile à suivre. Couper en deux. Première phrase "on a proposé une approche pour spécifier et vérifier un HSE au regard d'un model hardware (virer architecture). On a ensuite appliqué cette méthodo sur un exemple concret : le mécanisme d'isolation du BIOS utilisant la SMM.}
%
Such approach can benefit both hardware designers
and software developers.

First, the hardware model \TODO{A quoi renvoie précisément "hardware model" dans ton approche? harware\_req? software\_req? les deux? Il faut être explicite } can be used as a formal specification by
software developers. \TODO{tu es sur que c'est bien "software dev" que tu as voulu mettre ici? Dans ce cas je ne comprends pas trop ton développement. Tu dis que ton approche peut à la fois bénéficier au hard designer et soft dev mais ton "first" et ton 'secondly" s'adresse tous deux au "soft dev"...}
%
...
not specified if the first statement is true\,\footnote{If we had to actually
  ...
  correct one.}. \TODO{Cette phrase et la footnote correspondante ne sont pas claires.} \formatLTS{Minx86} as a formal specification does not suffer
from the same flaw. \TODO{Mouais, disons que par définition, une specification formelle ne peut être ambigu. Dans ton cas, tu as du faire un choix si j'ai bien compris. Mais est-ce le bon....?}
%
...
model impracticable in practice. \TODO{Que veux dire "monolithic model"? Il faudrait détailler un peu plus la problématique} 
%
