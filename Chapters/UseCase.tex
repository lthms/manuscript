...
\PC{avis perso: les 3 phrases précédentes me semblent à l'envers: je dirais
plutôt ``x86 est utilisé partout, Intel a mis plein de mécanismes, mais
ils on été contournés, du coup on a envie de les spécifier et vérifier''}
%
...
requires so it can play this role\PC{répétition (supprimer ``to play its
role'')} (Section~\ref{sec:usecase:firmware}).
%
...
Describing \PC{modern} hardware architectures in depth is challenging, because they tend to
comprise many interconnected components of various natures\PC{that were added
over time}.
%
...
command-line application \texttt{lshw} lists about 30 hardware components,\PC{je
ne suis pas sûr que la mesure lshw soit pertinente: elle ne détaille pas tout,
il y a beaucoup plus de composants en pratique. Tu peux dire que le PC est
composé de ``dozens of components'' :)}
which include a hard drive, a keyboard, a trackpad, an audio controller, an
...
l'exemple qui
suivent ?
Cela me semble un peu hors contexte, on se demande pourquoi ce point est
détaillé (et pas les autres)
...
\paragraph{Memories and Cores \IOs.}\PC{acronyme, donc I/Os ou IOs au pluriel}
%
...
\PC{après discussion d'hier, AMHA ici tu devrais faire une phrase pour
introduire les deux idées qui suivent: on distingue ce qui est accessible
\textit{via} l'espace d'adressage (mot important!) (le ``quoi'') et les manières
d'y accéder (le ``comment''), sachant que l'accès à une même ressource peut être
...
%
Other hardware components expose additional memory regions.\PC{techniquement, ce
	n'est pas le composant qui expose de la mémoire, mais le CPU (le memory
	controller) qui expose la mémoire ou les registres du composant dans
	l'espace d'adressage global. Il serait utile de citer le nom
	\textit{memory-mapped I/O} ici --- c'est cité plus loin, mais en
répétition}
%
...
Programmable \IOs (P-\IOs).\PC{jamais vu ce terme --- tu parle des
	\textit{Port-Mapped I/O (PMIO)}?}
%
In addition to this approach\PC{Proposal: ``Alternatively, peripheral
	memory and registers can be exposed to the system memory address space
	using \emph{memory mapped \IOs}.''

Le ``Alternatively'' est plus clair que ``In addition to this approach''}
, x86 \acp{cpu} are also capable of handling
\emph{memory-mapped} \IOs, where the memories exposed by the peripherals are
mapped within the system memory address space.
%
\PC{Phrase suivante inutile IMHO}
...
processor and its concrete memory location is called the \emph{memory
map}.\PC{``a memory mapping'' plutôt ? La ``memory map'' c'est l'ensemble de
tous les mappings}
%
...
%
\PC{saut de ligne: cette idée est importante, mais elle est noyée dans le
	paragraphe précédent. Il me semble aussi que ça part trop vite dans un
	exemple détaillé
	
	AMHA, il faudrait détailler plus l'idée générale, en expliquant
	le fait qu'une même zone mémoire ou registre d'un périphérique peut être
	lue ou modifiée par MMIO \textbf{et} PMIO, ce qui va notamment
	complexifier la modélisation,
	et réduire un peu l'exemple}
...
%
\PC{Je comprends l'idée de l'exemple, mais je ne suis pas persuadé que la
	fonction de calcul apporte quelque chose.
	Tu pourrais plutôt prendre un registre au hasard dans la figure et
	expliquer comment on y accède (relativement au BAR ---qui peut être
	modifé---, ou \textit{via}
	PMIO), \textbf{en citant la figure 2.2}}
...
Finally, cores do not \PC{usually} manipulate physical addresses directly.
%
...
by the software components.\PC{et la virtualisation, qui peut apporter les EPT}
...
%
When a user presses a key of its keyboard, the latter sends an
interrupt\PC{\textbf{request}}\PC{supprime le numéro d'IRQ (ça n'apporte pas grand chose, et c'est
	reconfigurable \ldots (\textit{cf.} P de PIC)}
(\texttt{\#IRQ1}).
%
...
%
When a core received\PC{handles} an interrupt, it saves its current context inside the
...
\texttt{\#IRQ14}\PC{supprime le numéro (qui me semble louche: il me semble que
tu as confondu IRQ et numéro du handler)}).

\PC{table 2.1: je confirme, ce n'est \textbf{pas} le numéro d'IRQ, mais celui du vecteur
d'interruption}
\begin{table}
  \centering
  \begin{tabular}{cl}
...
\PC{supprime le ``In addition'', et fais un paragraphe avec un titre}
...
All being told\PC{previously stated}, x86 computing platforms are formed of concurrent, interconnected
components.
%
...
to a key hierarchy\PC{supprimer la suite (les clés ne sont pas nécessairement
configurables} configurable by the computing platform owner.
%
...
zones, adjusting \acp{cpu} speed.\PC{and configuring hardware workarounds}
%
...
\ac{bios} is to keep the computer in a working state.\PC{ensure that the
software stack cannot harm the computer}
%
...
qui est impliquée dans le hse ?!} in the \ac{hse} mechanisms
...
\PC{(fond): ce texte inter-paragraphe redit surtout les choses}
...
volatile memory.
\PC{Cette notion de \textit{secure channel} est-elle importante~?
Si oui, ajoute un emph et sors-là de ce paragraphe, IMHO}
%\PC{Cette notion de \textit{secure channel} est-elle importante
%? Si oui, ajoute un \verb+\emph+ et sors-là de ce paragraphe, IMHO}
