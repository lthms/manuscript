...
\begin{definition}[Software Execution Isolation Policy]
  ...
  \[
    \begin{array}{l}
      I(h, l, h') \triangleq
      \forall (x, y) \in T \times T \text{, } h \xrightarrow[y
...
\begin{corollary}
  ...
  %
  \[
    I(h, l, h') \iff \forall (x, y) \in T \times T \text{, } y \not\ge x
...
mechanisms.
%
We can break it down to more specific security policies, by designating the set
of security components whose executions $U \subseteq T$ has to be constrained
with respect to the global security policy $I$.
%
\[
  I|_{U}(h, l, h') \triangleq \forall u \in U, \forall x \in S, h \xrightarrow[u
  \leadsto t]{l} h' \Rightarrow u \ge x
\]
%
This safety property can only be enforced through a HSE mechanism which shares
its hardware-software mapping with $I$, and whose \ac{tcb} is
$T = \{ t \in S\ |\ t \not\in U \wedge \forall u \in U, t \ge u \}$.

\begin{example}[Constraining Applications]
  For
  ...
  %
  Indeed, the responsibility to constrain the execution of applications cannot
  fall entirely on system software.
  %
  ...
  \ac{tcb} is $T = \{ \mathtt{bios} \}$. Only the \ac{bios} is responsible for
  constraining the execution of system software.
  ...
  \[
    \begin{array}{cl}
      \Delta_{\mathtt{bios}}
      & \models \func{safety\_property}_{I|_{U_{\mathtt{os}}}} \\
      \Delta_{\mathtt{os}} & \models
                             \func{safety\_property}_{I|_{U_{\mathtt{apps}}}}
    \end{array}
  \]
  ...
  \[
    \begin{array}{l}
      \forall \rho \in \pathesLTS{Ez}, \\
      \qquad \rho \in \mathcal{C}(\Delta_{\mathtt{bios}}) \cap
      \mathcal{C}(\Delta_{\mathtt{os}}) \\
      \qquad \Rightarrow \forall tr \in \func{trans}(\rho), I(tr)
    \end{array}
  \]
\end{example}
