...
\vspace{1cm}\noindent
%
\PC{je suis toujours dubitatif sur cette première phrase. Je suis surpris que tu
  ne parle pas de \textbf{modèle} (en introduisant le concept) au lieu de
  système. Le modèle peut correspondre ou non à l'implémentation réelle}
%
...
%
%In particular, we do not cover \emph{model validation}, that is the verification
%that a model is a correct abstraction of the concrete system it supposedly
%describes.
%
...
%
\thomasrk[inline]{Mieux vendre que c'est un exemple simple et qu'on peut
  facilement tirer.}
%
...
%
In order to verify \ac{hse} mechanisms, it is important to consider that
%
\begin{inparaenum}[(1)]
...
We now give an overview of formal verification projects which have required to
define a functional specification of (part of) a hardware architecture, either
to reason about the architecture itself (\ref{subsubsec:sota:hw}), or to verify
the correctness of software components executed by this architecture
(\ref{subsubsec:sota:ssw} and \ref{subsubsec:sota:asw}).
%
We detail the projects scope, the verification approaches authors utilized, and
we discuss to what extent their results apply to our objectives.

\subsubsection{x86 Models}
...
the years, but for purposes of formally reasoning about machine-code programs.
%
...
%
For instance, Gu \emph{et al.} extended CompCert to reason about
concurrency\,\cite{gu2016certikos}, and Chen \emph{et al.} developed an approach
to reason about interruptible programs\,\cite{chen2018interrupt}.
%
However, this requires an important level of expertise and understanding of
fairly complicated formal developments.

As a consequence, we decided to develop an ad-hoc x86 hardware model which
include the necessary hardware components ---abstracted away in related work---
to demonstrate in Chapter~\ref{chapter:speccert2} how it is possible to leverage
our formal definition of \ac{hse} mechanisms presented in
Chapter~\ref{chapter:speccert} in order to specify and verify a \ac{hse}
mechanism implemented by the \ac{bios}.

\subsubsection{Hardware Architecture Model}
\label{subsubsec:sota:hw}
...
%
However, the state explosion problem forces the authors to simplify their model,
in order to reduce the state combinatory.  \PC{ça manque un peu d'évaluation: la
  seule phrase donnant les limites de l'approche est la dernière: il faut
  montrer pourquoi ce qui existe ne suffit pas!}

\subsubsection{System Software Components Verification}
\label{subsubsec:sota:ssw}
...
%
\PC{trop générique/vague: ``une analyse statique ?'' laquelle ? pour faire quoi}
...
to reason with.  \PC{idem, on ne voit pas les limites de ce travail, et/ou
  pourquoi tu ne l'utilise pas directement}
...
\subsubsection{Machine-Code Program Verification}
\label{subsubsec:sota:asw}

\paragraph{RockSalt.}
%
A Google service called \ac{nacl} allows for for software developers to
distribute their web applications in the form of native executable code.
%
...
%
In practice, \ac{nacl} checks that an arbitrary native applications respects the
\ac{sfi} rules before loading it to the browser context.
%
As a consequence, the browser is protected from malicious machine code.

From a security perspective, this means there are two requirements over
\ac{nacl}:
%
\begin{inparaenum}[(1)]
...
does not make them vulnerable to compositional attacks.
%
...
%
\PC{saut de ligne} Another popular modeling structure is the Moore Machine.
%
