...
\item [Firmware/BIOS]
  ...
  %
\item [System Software] \PC{Tu parle bien de l'OS ? Pourquoi ne pas l'appeler
    par son nom, ou au minimum dire que le System Software est l'OS dans la
    plupart (tous?) des cas ?}
  %
...
\item [Applications]
  ...
  Applications leverages the high-level interface exposed by \PC{(un peu
    partout): \textbf{the} system software} system software in order to provide
  a service to the user of the computing platform.
  %
...
%
For instance, \PC{the} system software manages the life cycle of the
...
backdoors are no fantasy\PC{slang}\,\cite{yang2016a2}, the \ac{bios} has been
used as an attack vector to tamper with its execution even after a machine
...
\PC{le paragraphe suivant est un peu confus: tu parle de différente manières de
  restreindre, mais la phrase 3 commence par un ``in a nutshell'' qui est une
  idée générique (et pas une de ces manières), après tu as un exemple, etc.}
...
\PC{(transition) AMHA, il manque une phrase ici pour dire pourquoi tu as besoin
  de la définition qui suit. QQch comme ``To achieve security, each layer has to
  know and use properly the security mechanisms offered by the underlying
  layers'' ?}
...
\begin{definition}[Hardware-based Security Enforcement]
  ...
  software component configures the underlying hardware architecture to
  constrain the execution of untrusted software components with respect to a
  targeted security policy.\PC{``the class of mechanisms where a \ldots
    component configures the hardware'' ça fait un peu bizarre à la lecture}
\end{definition}
...
components are often due to some misuse of hardware features\thomasrk{une
  citation serait bienvenue}.
%
...
%
It is important to notice that most of them have been circumvented due to
...
%
\PC{(style haché)fusionner avec la phrase précédente: ``protected \ldots based
  on a particular hardware interrupt''}
%
...
This did not prevent Intel to introduce TXT\,\cite{intel2015txt},\PC{on a
  l'impression que l'erreur a été d'introduire TXT! il faudrait mettre le
  problème en début de phrase} a novel security mechanism which had the
particular side effect of disabling this hardware
interrupt\,\cite{kovah2015senter}.
%
...
%
Compositional attacks are \PC{often} due to a flaw in the specifications of the
computing platform.
%
...
%
Also, the same hardware component can be found in different hardware
architecture to play a similar role.
%
...
%
Otherwise, each modification of the hardware architecture will have an
important impact on the proofs already written.

\paragraph{}
%
While we have tried to propose generic formalisms which can potentially be
applied to large ranges of problems, we have decided to systematically implement
and evaluate them on the x86 architecture.
%
...
\item The extraction mechanism of Coq allows for transpiling formal
  ...
  their implementation, \PC{(suite phrase) mal formulé}this possibility is worth
  taking into account.
\end{itemize}
