...
Having reasoning about the correctness of \( \Delta_{\mathtt{bios}} \), we have
identified three limitations which reduce the applicability of {\scshape Minx86}
as we would increase its scope: its monolithic design, its weakness against the
temporary violation problem\,\cite{muller2006tempviolation}, and the presence
within our supposedly general-purpose model of abstract values specific to a
...
successfully managed the limitation of {\scshape Minx86} partly because we have
taken great care in organizing our development as modurarly as we could.
%
We now detail the approach we used.
...
%
To that end, we leverage
%
\thomasrk[inline]{TODO, décider si on reste vague ici ou si on parle plus
  précisément du fait qu’on utilise un paradigme bien précis qui nous permet de
  le faire (choisir le niveau de détail, en soit)}
...
We now give an overview of the formalism that we propose as a solution to
overcome {\scshape Minx86} flaws, both to model and verify a hardware
architecture.
%
Its key concept is to focus of components \emph{interfaces}, where an
interface is a set of interdependent operations which are expected to produce a
value.
...
both model verify the x86 hardware architecture.
%
