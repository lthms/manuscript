...
%
As a consequence \TODO{je ne vois pas trop le lien de cause à effet. Revoir la
  transition de cette phrase}, we have broken down our formal definition into
three components: hardware models, \ac{hse} mechanisms and security
policies. \TODO{Attention à l'usage des temps. Dans cette phrase tu utilises le
  present perfect. Ensuite c'est le présent...}
%
...
proof sketches which follow. \TODO{Je ne suis pas sur de comprendre précisemment
  la dernière phrase.}
...
state of the next one. \TODO{Tu ne donnes qu'une définition informelle (en
...
As we detailed in Subsection~\ref{subsec:state:secu} \TODO{le label de cette
  sous-section n'est pas valide}, security policies are modelled as predicates
...
wherein the requirements over state are not satisfied. \TODO{Tu devrais dès le
  début bien insister sur le fait que ces contraintes ne s'applique que sur le
  code trusté. On va supposer que le code trusté utiliser correctement le
  mécanisme hardware. Dans notre modèle d'attaquant, l'attaquant n'a que le
  controle du code non trusté, qui n'est donc pas contraint.}
%
...
mechanism. \TODO{Peut être rappeller que "correctly implement the HSE" veut dire
...
\paragraph{Definition.} \TODO{Le nom du paragraphe (Definition) n'est pas
  top. Il y en a plein partout, des définitions. Je propose quelque chose du
  style "HSE mecanisms"...}
%
...
transitions. \TODO{Cela devrait être expliqué un peu plus en détail et plus
  tôt. Il suffirait alors ici de le rappeler (comme expliqué en section xx, ces
  contraintes ne concernent que les transition trustés.}
%

Dealing with multi-core architectures is possible, but requires additional
efforts. \TODO{J'ai l'impression que cela devrait être mentionné et discuté plus
  haut. De manière générale, comment modéliser une archi multi-core? Plusieurs
  transitions peuvent-elles arriver simultanément?}
%
One solution is to define a set of core identifiers to act as an additional
parameter for hardware-software mapping and software transitions. \TODO{Pourquoi
  seulement sur les transitions software? Si un coeur fait un défaut de page,
  est-ce que l'interruption est transmise à tous les coeurs?}
%
However, this is out of the scope of this Chapter. \TODO{C'est du bottage en
  touche peu élégant. Je dirais que c'est une limite du modèle tel qu'il est
  présenté dans cette thèse. Mais on pourrait étendre le modèle pour traiter le
  multi-coeur. Il faut le dire clairement. C'est pas un truc hors sujet du
  chapitre (sauf erreur de ma part, le problème n'est pas traité ailleurs dans
  le manuscrit). }
...
We illustrate this definition with the flash memory lockdown mechanism
...
\item In order to protect the integrity of the flash memory content, the
  ...
  instruction. \TODO{Avant de faire un lock, je suppose que le code du firmware
    fait quelque chose d'utile (mettre à jour le firmware?) Il faudrait le
    préciser.}
\end{enumerate}
...
\begin{example}[Flash Memory Lockdown Mechanism]
  ...
  In this example, the trusted software component ($T$) is the \ac{bios}. \TODO{
    et du coup, il faudrait préseciser que les autres composants (OS,
    applications, etc.) sont les non-trustés.}
  %
  The \ac{cpu} executes the \ac{bios} when it is in \ac{smm}
  (\func{context}). \TODO{La phrase n'est pas suffisamment préciser. En outre,
    j'ai mis du temps à me rappeler que context ne doit pas être pris ici comme
    un mot commun mais un nom de fonction!. Il faudrait dire que par défaut
    contexte(h)=non trusté sauf lorsque le CPU est en SMM, c'est à dire
    lorsqu'une SMI se produit et avant que le bios exécute une instruction rsm}
  %
...
\begin{example}[Flash Memory Lockdown Inconsistency]
  ...
  It is because the flash memory lockdown as described here does not satisfy
