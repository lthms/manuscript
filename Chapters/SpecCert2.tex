...
First, we formally specify a targeted security policy for this \ac{hse}
mechanism.
%
The purpose of this security policy is to prevent system software to make the
\ac{bios} execute an assembly instruction of its choice.
%
We take the approach to propose a generic class of security policies we latter
...
\section{Specifying Isolation Policies}
...
\begin{example}[SMRAM Cache Poisoning]
  ...
  instructions within the cache for the \ac{cpu} to update its \texttt{SMBASE}
  register.
\end{example}
...
\begin{definition}[Global Execution Isolation Policy]
  ...
  a tuple $\langle S, \rightarrowtail, \func{context} \rangle$, such that
  %
  \begin{itemize}
  ...
  \item $\rightarrowtail$ is a binary relation on $S$, such that given
    ...
    achieve a code injection against $y$. $\rightarrowtail$ has to be
    %
    \begin{inparaenum}[(1)]
  ...
  when code injections which occur during this transition are legitimate
...
\begin{example}[Software Stack Isolation]
  ...
  We define the authorization relation $\rightarrowtail$ using three deductive
  rules:

  \begin{minipage}[c]{0.3\linewidth}
    \centering
    \begin{prooftree}
      \AxiomC{$x \in S$} \RightLabel{\scriptsize{$\rightarrowtail$-refl}}
      \UnaryInfC{$x \rightarrowtail x$}
    \end{prooftree}
  \end{minipage}
  %
  \hfill
  ...
  \begin{minipage}[c]{0.3\linewidth}
    \centering
    \begin{prooftree}
      \AxiomC{$y \in S$} \RightLabel{\scriptsize{$\rightarrowtail$-bios}}
      \UnaryInfC{$\mathtt{bios} \rightarrowtail x$}
    \end{prooftree}
  \end{minipage}
  %
  \hfill
  ...
  \begin{minipage}[c]{0.3\linewidth}
    \centering
    \begin{prooftree}
      \AxiomC{$0 \le k \le n$} \RightLabel{\scriptsize{$\rightarrowtail$-os}}
      \UnaryInfC{$\mathtt{os} \rightarrowtail \mathtt{app}_k$}
    \end{prooftree}
  \end{minipage}
...
%     \end{tikzpicture}
%   \end{center}
%   \caption{Representation of the authorization relation $\rightarrowtail$
%   defined in Example~\ref{example:speccert:ssisolation}.}
...
\begin{definition}[Isolation Sub-policies]
...
\paragraph{Hardware Architecture State}
%
...
\paragraph{Transition Labels and Transition Relation}
%
...
\section{HSE Mechanisms Concurrent Implementation}
...
\begin{lemma}[Compliant Traces of Intersection]
  ...
  \begin{proof}
    ...
    of traces and \func{inv}, $R$ is the premise filtering software transitions,
    and $Q$ and $Q'$ are statements about \func{behaviour}.
  \end{proof}
\end{lemma}
