...
core\,\footnote{As discussed in the previous Chapter, we did not tackle the
  challenge posed by multi-core computer in this thesis.}, one level of cache, a
...
\paragraph{Memories.}
%
The physical memories (exposed by the DRAM and the VGA controllers) are modelled
...
\GH{je viens de réaliser que tu as renommé les "hardware requirements" en
  "requirement over states". Outre que le nom devient long je ne suis pas sur
  qu'il ne reste pas quelques mentions de hardware requirement dans le
  manuscrit. Il faut vérifier}
...
so there is no untrusted software component execution whose execution need to be
constrained.
%
...
%
Otherwise, the runtime will not start from a ``safe'' states and the related
...
independently from the correctness of \( \Delta_{\mathtt{bios}} \).
