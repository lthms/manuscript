...
\item [System Software]
  ...
  (\emph{e.g.} system calls), so that applications can focus on business logic. Thus, applications often 
  do not care about which particular hardware architecture
  they are executed on.
  %
  In addition, system software deal with the complexity implied by
  the concurrent execution of several applications.
  %
\item [Applications]
  %
  \TODO{Il faudrait définir positivement les applications : ce sont elles qui offre un service à l'utilisateur}
...
migrations. \TODO{ajouter des références. De quels hyperviseurs veux tu parler? c'est un peu flou}
%
It is also frequent that application internals are themselves built as
abstraction layers (\emph{e.g.} applicative framework, core logic, plugins, etc.).

\TODO{Rajouter une jolie figure pour illustrer cet empilement de couches et y faire référence dans le texte}
...
\TODO{La phrase précédente est beaucoup trop longue et confuse. On ne voit pas ce que tu veux dire au final. Fait plusieurs phrase. Si j'ai bien suivi, tu veux dire 1) que chaque couche fait confiance à la précédente. Mais que 2) cette confiance est discutable car l'histoire montre que des vulnérabilités ou des portes dérobées existent dans chacune des couches. Ecrit 1), donne un exemple puis 2 et donne un exemple mais ne mélange pas tout.}

%
One layer may constrain its successors' execution \TODO{the execution of its successors? J'ai l'impression qu'on ne voit pas trop la construction somethings' blabla dans des articles techniques et scientifiques. cf \url{https://ell.stackexchange.com/questions/157380/whats-the-correct-usage-of-something-of-something-and-somethings-something} }, with respect to targeted
security properties.
%
...
%
For instance, system software often leverages among other mechanisms a
\ac{mmu} to partition the system memory.
%
Thus, when an application is executed, it can only access a portion \TODO{a subset?} of the
system memory.
%
...
\begin{definition}[Hardware-based Security Enforcement]
  ...
  the execution of untrusted software components.
\end{definition}
...
as expected.\TODO{En relisant, c'est cette première phrase qui n'est pas claire. Je pense que tu voulias dire que pour que le HSE assure bien la propriété de sécurité, cela suppose deux chose :1) qu'il soit correctement configuré pas le code trusté et 2) que les fonctionnalité hardware fonctionne comme espéré. Il faudrait mieux faire ressortir cela. Ne pas hésiter à faire une énumération explicite.}
%
Both remain challenging. \TODO{Cette dernière phrase n'est pas claire. A quoi renvoie "both"?}
...
risk.
%
\TODO{Je propose de virer les deux phrases précédentes qui sont redondantes avec ce qui précède et sont plutôt confuses}
...
components\,\cite{bulygin2014bios}. \TODO{"many security vulne" $\rightarrow$ on s'attend à avoir plusieurs références}
%
