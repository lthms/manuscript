...
%
Nowadays, it contains two or more \emph{cores}, which execute software
components programs independently thanks to a so-called \emph{pipeline}.
%
...
%
To execute instructions such as \texttt{add}, a core comprises several hardware
components specialized in very specific tasks (\emph{eg.} fetch the instruction,
decode the instruction, general-purpose or configuration registers, etc.).
%
They are connected together to form a \emph{pipeline}.
%
Cores (and pipelines) are independent from each other.
%
...
logical cores.
%
Two logical cores share the pipeline of their physical core, but the hardware
implementation supposedly keeps them isolated.
%
