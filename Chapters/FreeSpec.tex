...
mechanisms and have leveraged this theory to specify and verify the \ac{hse}
mechanism implemented by the \ac{bios} at runtime in order to remain isolated
from the rest of the software stack.
%
In this context, we have defined a small x86 hardware model called {\scshape
...
along with a companion framework for Coq to support this
approach\,\cite{letan2018freespeccode}.

The rest of the Chapter proceeds as follows.
%
We justified our contribution with the experience implementing the proofs of
...
effect handlers (Section~\ref{sec:freespec:specifying}) and we introduce
so-called abstract specifications to verify their respective properties
...
%
On the one hand, $\func{Pure}$ and
$\func{Request}$ are comparable to the leaves of a syntax tree and model atomic
computations; $\func{Pure}$ models local computations, whereas
$\func{Request}$ models deferring a computational request to a handler and
waiting for its result.
%
On the other hand,
$\func{Bind}$ (usually written thanks to the infix operator
...
\begin{example}[Copy]
  ...
  P_{\I_{\mathrm{DRAM}}}(\{\ac{unit}\})$ such that $\func{copy}(l,
  l')$ models a program with effects that returns no result, but copies the
...
accesses do not lead to any abstract state update.
%
...
\begin{example}[Memory Controller and IOMMU]
  ...
  %
  \[
    \func{pnf}_{\mathrm{IOMMU}}(e) \triangleq \forall (l, v) \in \mathpzc{Loc}
...
Figure~\ref{fig:freespec:interfacedriven}, using FreeSpec requires to take
