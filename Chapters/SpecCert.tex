...
%
This case study is used as the basis for a more in-depth experiment in the next
Chapter.

For readers familiar with Coq, we present an implementation of our theory in
...
%
Besides, it makes reasoning about \ac{hse} mechanisms composition ---that is, the
...
components, as a particular \ac{lts}
(Definition~\ref{def:sota:lts})\,\footnote{On page~\pageref{def:sota:lts}.}
with two sets of labels instead of one.

\paragraph{States.}
%
The set of states of the \ac{lts} models possible configurations of the
hardware components.
%
...
which are direct and foreseeable side effects of the execution of instructions
...
Mainstream hardware architectures are not ``safe'' by default, and require
...
%
We aim to model \ac{hse} mechanisms as subsets of traces, to discard traces that
are legitimate from a hardware specification perspective, yet dangerous from a
security perspective.

\subsection{HSE Mechanisms}
\label{subsec:speccert:hse}

In our theory, a \ac{hse} mechanism is primarily characterized by a set of
trusted software components, a set of requirements over states and a set of
requirements over software transitions.
%
Trusted software components are responsible for implementing the \ac{hse}
mechanism.
%
To that end, they have to correctly configure the underlying hardware mechanism
in accordance with the two requirements over states and over software
transitions.
...
first law is incorrectly making assumptions about ``untrusted'' software
components.
%
...
%
In such a condition, the hardware architecture may stop constraining the
...
%
If the requirements over hardware states cannot be loosened without threatening
the security policy enforcement or finding restrictions over trusted software
execution, then the \ac{hse} mechanism probably does not serve its purpose.
...
\begin{example}[Flash Memory Lockdown Inconsistency]
  ...
  %
  This has to be modeled by a software transition.
  %
  Then, the \ac{pch} triggers a \ac{smi}, leading the \ac{cpu} to enter
  \ac{smm}.
  %
  A \ac{smi} has to be modeled as a hardware transition.
  %
...
\ac{hse} mechanism.
%
The purpose of these requirements over software transitions is to prevent
untrusted software components to reach a hardware configuration wherein the
requirements over states are not satisfied. This implies that the initial state
of the trace must satisfy the requirements over states.
%
...
\begin{definition}[Compliant Traces]
  \label{def:speccert:complianttraces}
...
\begin{example}[Memory Flash Lockdown Compliance]
  ...
  %
  If the \ac{bios} fails to do so, then the related trace is not compliant to
  begin with.
  %
  If it succeeds, then it still needs to carefully satisfy its requirements over
  transitions throughout its execution for the trace to remain compliant over
  time.
\end{example}

\begin{lemma}[HSE Invariant Enforcement]
  \label{lemma:speccert:hseinv}
  As intended, \func{hardware\_req} is an invariant of traces which comply
  with $\Delta$, that is
  %
  \[
...
states is only a means to an end.
%
...
%
Back then, the isolation of the \ac{bios} at runtime was enforced by only two
...
%
The rest of the software component can try to read from or write to the SMRAM
(first law), but the memory controller will protect its integrity in the presence of
a core not in \ac{smm} (second law).
%
However, between the core and the memory controller lies the cache, as pictured
in Figure~\ref{fig:speccert:smram}.
%
If the cache does not implement an access control mechanism to protect its
copies of the SMRAM content ---as was the case in 2009--- it can be used to
circumvent the protection enforced by the memory controller.
%
As we have already explained in Subsection~\ref{subsec:usecase:hse:smram}, the
integrity of the SMRAM is enforced, so only a core in \ac{smm} can access to the
content of the SMRAM.
%
However, when this core leaves the \ac{smm}, the content of the SMRAM remains in
the cache, and nothing prevents an untrusted software component to modify these copies.
%
When the core enters in \ac{smm} again, it fetches the (modified) content of the
cache in place of the true SMRAM content.
...
Hardware features leveraged in the context of a \ac{hse} mechanism \( \Delta \)
are not safe by default and require to be configured by trusted software
components.
%
As a consequence, the security policy \( P \) targeted by \( \Delta \) does not
apply to the set of all traces of the hardware model, but we expect that
satisfying the requirements of \( \Delta \) ---which is modeled in our theory by
the set of compliant traces \( \mathcal{C}(\Delta) \)--- is a sufficient
...
%
To the extent of our knowledge, the majority of \ac{hse} mechanisms target
access control policies, which qualify as safety properties.
%
The reasoning is facilitated because they are characterized by predicates on
transitions.
%
...
\begin{theorem}[Correct HSE Mechanism for Predicate on Transitions]
  ...
  \begin{proof}
    By definition, a HSE mechanism \( \Delta \) is correct with respect to a
    ...
    %
    Therefore, if we can prove that transitions which satisfy both
...
A proof of the correctness of a given \ac{hse} mechanism asserts that the hardware
architecture enforces a targeted security policy \emph{if trusted software
  components correctly implement this \ac{hse} mechanism}, that is their
executions only imply software transitions which satisfy the appropriate
requirements.
%
If attackers are able to modify the instructions of the trusted software
components programs, they can defeat the security enforcement by modifying the
instructions responsible for implementing it.
%
% \GH{ce n'est pas le seul risque (même si c'est le plus important). De manière
% générale, si un composant non trusté peu influencer le comportement d'un code
% trusté (par exemple, en perturbant son flot d'exécution), il peut l'amener à
% ne plus respecter les software req. Mais c'est le problème du software
% trusté. Il doit être correct ET robuste aux attaques.}
%
% Thomas : en fait, j’ai l’impression que sans rien ajouter, c’est finalement
% assez clair.
%
...
%
In other words, to comply with the composition of two \ac{hse} mechanisms, a
...
\begin{lemma}[Commutative Monoid]
  ...
  \begin{proof}
    ...
    %
    For each property, the proof consists of unfolding the definition of
...
\( \Delta_2 \) can doubtlessly be considered incompatible.
%
...
traces where the execution of trusted software components are secure, but is not
able to serve its purpose.
%
We argue that a similar situation can occur for a single \ac{hse} mechanism.
%
Because we aimed to focus on hardware architecture verification in the context
of this thesis, we therefore did not investigate further the compatibility of
\ac{hse} mechanisms.

\section{Case Study: Code Injection Policy}
\label{sec:speccert:casestudy}

In the previous Section, we introduced our theory to formally specify and verify
\ac{hse} mechanisms.
%
It allows us to verify each \ac{hse} mechanism in isolation, and we also
...
\begin{definition}[Transition-Software Mapping]
  ...
  of owners of the instructions read during this transition by the core in order
  to execute them afterwards.
  %
  The word ``fetch'' is used in the hardware industry to describe this subset of
  read accesses targeting instructions.
\end{definition}
...
\begin{example}[Application Code Injection]
  When a user executes a new application, the operating system loads the code of
  this application into the \ac{dram}.
  %
...
On the contrary, an illegitimate code injection poses a significant threat
against the security of the software stack, because it constitutes an attack
...
code injection policy. This policy determines if a given software component is
authorized to make a code injection against another software component.
...
\begin{definition}[BIOS-OS-Applications Code Injection Policy]
  ...
  For a software stack made of a \ac{bios}, an operating system and \( n \)
  ...
  processor is not in \ac{smm} and is in ring 0, the operating system is
  executed; when the processor is not in \ac{smm} and is in ring 3, one of the
  applications ---identified by the page tables used by the processor--- is
  executed.
\end{definition}
...
one being enforced by a dedicated \ac{hse} mechanism.
...
\begin{theorem}[Code Injection Policy Enforcement]
  \label{theorem:speccert:compoenforced}
  ...
  \begin{proof}
    ...
    \item Because of \( I_{\mathtt{os}} \), an application can only perform a
      case injection against itself, which also satisfies \( I \). \hfill
      \( \square \)
    \end{itemize}
  \end{proof}
\end{theorem}
...
In this Section, we specified a general-purpose policy called code injection policy,
...
%
We emphasize that our approach can be used to verify several \ac{hse} mechanisms
...
interfere with each other, as illustrated by the \texttt{SENTER} Sandman attack discussed
