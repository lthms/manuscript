...
\begin{example}[MCH Interfaces]
  ...
  %
  We use the unit type \ac{unit} to model effects without results (similarly to
...
\begin{example}[VGA and DRAM Composition]
  ...
  \Interface{(\I_{\mathrm{DRAM}} \oplus \I_{\mathrm{VGA}})}{\{()\}}$ is an
  effect that describes a write access targeting the VGA controller.
\end{example}
...
\begin{theorem}[Non-Interference and Concurrency]
  ...
  $\mathcal{I}$. Let $\func{pnf} : \I \rightarrow \texttt{Prop}$ be a subset of
