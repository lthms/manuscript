...
\item The extraction mechanism of Coq allows for transpiling formal
  ...
  implementation. \TODO{Tu pourrais rajouter une footnote disant que bien qu'il s'agisse d'une propriété intéressante qui a notamment motivé ton choix,  tu ne l'as pas utilisée durant ta thèse}
  %
  ...
  confidence in a verification result. \TODO{Comme tu n'as pas exploité cette fonctionnalité, j'éviterais de trop insister donc je supprimerais cette dernière phrase. De toute façon, cette fonctionnalité aurait également un intérêt si tu travaillez en amont, pour vérifier par la suite que les implémentations sont cohérente avec la spec (dans ce cas, ce n'est pas la spec que tu valides, mais l'implémentation}
\end{itemize}
