%!TEX root = ../main.tex
\chapter{Formal Verification With Respect To Security Policies}
...
%
To the best of our knowledge, the more straightforward definition\PC{ce début de
  phrase laisse penser que 1. tu n'es pas sûr 2. il n'y a pas de def standard ?
  - à revoir} of a labeled transition system, as used by Vijayaraghavan \emph{et
...
initial states.\PC{et ? (phrase qui tombe un peu de nulle part)}

\PC{l'exemple Airlock Systems tombe carrément du ciel, on ne voit pas pourquo tu
  le choisis. Je suggère de mettre ici une phrase disant que tu vas prendre un
  exemple récurrent pour illuster, et pourquoi tu le prends}

\begin{example}[Airlock System]
  \label{example:sota:airlocklts}
  Figure~\ref{fig:sota:airlock-lts} illustrates this definition for a simple
  airlock system.
  %
  ...
  \item A transition is characterized by the opening (\( \mathtt{Open}_i\), with
    ...
    %
  \item The system does\PC{should?} not allow the simultaneous opening of both
...
There are many modeling structures for characterizing transition
...
together.
%
Such properties\PC{lesquelles ? il y a un mix entre policies et properties} are
called \emph{hyperproperties}\,\cite{marr2002hypertheading},
...
\PC{ça manque un peu d'évaluation: la seule phrase donnant les limites de
	l'approche est la dernière: il faut montrer pourquoi ce qui existe ne
suffit pas!}
...
naive\PC{un peu agressif. ``simple / trivial'' ?}
...
to reason with.
\PC{idem, on ne voit pas les limites de ce travail, et/ou pourquoi tu ne
l'utilise pas directement}
