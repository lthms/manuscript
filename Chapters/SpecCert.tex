...
\begin{definition}[Transition-Software Mapping]
  ...
  $\func{fet\-ched}: H \times L \rightarrow \powerset(S)$ is a function which
  takes an initial hardware state, a transition label and returns the set of
  owners of the fetched instructions during this transition.
\end{definition}

Hence, $s \in \func{fetched}(h, l)$ means that an instruction owned by $s$ was
fetched during a transition labelled with $l$ from a state $h$.
%
...
\begin{definition}[Software Execution Tampering]
  ...
  Given $h$ the initial state of a transition labelled with $l$,
  $\func{context}$ a hardware-software mapping, $\func{fetched}$ an
  event-software mapping and $(x, y) \in S \times S$ two software components,
  the software component $y$ tampers with the execution of another software
 ...
 execution isolation policy prevents the untrusted components from tampering
 with the execution of the trusted components.
 %
 ...
 \begin{definition}[Software Execution Isolation Policy]
   ...
   $\langle \func{fetched}, \func{context}, T \rangle$, such that \func{fetched}
   is a transition-software mapping (specific to a hardware model),
   \func{context} is a hardware-software mapping, and $T$ is a set of trusted
   software components.
   %
   \[ \begin{array}{l}
        \func{software\_execution\_isolation}_I(h, l, h') \triangleq \\
        \forall t \in T, \forall u \not\in T, \\
        \qquad\qquad \neg \func{software\_tampering}(\func{context},
        \func{fetched}, h, l, t, u)
      \end{array} \]
  \end{definition}
...
%
  \begin{inparaenum}[(1)]
  \item specifying the software requirements that must be satisfied by the
...
%
  ...
  \end{inparaenum}
%
  A \ac{hse} mechanism specification is sound when untrusted software
  components' executions are constrain with respect to the security property as
  long as the trusted software component correctly implements it.
%
...
%
  ...
  SpecCert formalism.
%
  In Section \ref{sec:speccert:hardware}, we define a model of x86-based
  hardware architectures to verify \ac{hse} mechanisms targeting software
  isolation policies using publicly available Intel specifications.
%
