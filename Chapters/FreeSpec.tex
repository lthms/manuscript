...
Figure~\ref{fig:freespec:remember}). \GH{Il faudrait donner le langage utilisé. En outre, c'est un peu dommage de ne voir ce genre d'extrait que pour illustrer des limites. Tu pourrais peux-être placer un ou deux extrait du même genre pour illustrer des preuves que tu décris dans le chapitre 5.}
...
\begin{example}[Temporary Violation Problem in an Airlock System]
  ...
  %
  We did that by avoiding transitions between states \GH{attention, en anglais fermé se dit "closed" et pas "close" qui veut dire "pres de "}
...
a mistake \GH{mistake est un peu trop fort. "ne satisfait pas pleinement" ou "ne respecte pas complètement"...} with respect to our objective to rely on general-purpose models to
...
accompanied by a \texttt{Architecture\_proofs.v} file. \GH{Je ne comprends pas ce que tu veux dire avec cette dernière phrase (pas de soucis avec la précédente, en revanche)}
...
preserve the effects results sets. \GH{A ce stage on ne comprend pas pourquoi on a 2 constructueurs, ni la partie "defined to
preserve the effects results sets".Trop d'implicite, il faut détailler un peux}
%
...
\begin{example}[VGA and DRAM Composition]
  ...
  an effect that describes a write access targeting the VGA controller. \GH{comme dans ton exemple tu changer à la fois l'effet (read ou write) et l'interface, on ne saisi pas ce qui justifie qu'on ait utilisé InL ou InR. Il faudrait ne faire varier que le paramètre important pour bien insister. I et J se ressemble beaucoup dans ta définition 6.2 donc il faut être super attentif. L'exemple devrait permettre d'insister}
\end{example}
...
\begin{definition}[Operational Semantics
  ...
  functions. \GH{est-ce vraiement indispensable? Il y a déjà beaucoup de notions assez diverse à maitriser pour comprendre ton document. Si on peut s'éviter celle là...}
  %
...
% ``changes.''
%
Handling an effect of $\I$ (respectively $\J$) does not interfere \GH{plutôt que interfere, je préfererais "update" qui est plus proche de ce qui se passe.} with
$\sigma_j$ (respectively $\sigma_i$).
%
As a consequence, \emph{we cannot specify as-is a graph of components which
  contains a cycle or a diamond}. \GH{il faudrait s'attarder un peu plus là dessus et expliquer pourquoi a partir d'un exemple (le diamand, par exemple). Cela permettrait d'ailleurs d'expliquer un peu mieux la composition d'interface à travers un exemple}
%
