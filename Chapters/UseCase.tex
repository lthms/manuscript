...
\ac{bios} remain isolated from the rest of the software stack at runtime
...
%
We detail the mechanisms which play a part in interactions between the
...
platform.
%
Cores interact with these memories during so-called \IOs, for Input/Output \GH{tu as utilisé le terme I/O avant}: a
...
interpreted as an operand of an arithmetical operation. \GH{and then? En outre, c'est un exemple parmi d'autre, non?}
%
...
x86 instructions (\emph{e.g.} \texttt{mov} variants, arithmetical operations such
as \texttt{sub} and \texttt{add}) are designed to manipulate it.
%
Addresses of the system memory are often referred to as physical addresses, and the
majority of the system memory is made of \ac{dram}. \GH{pas très claire.}
%
...
When a core successfully reads the memory at a given address, it keeps a
...
cell if necessary. \GH{fait 2 phrases. une pour décrire cache eviction, une pour décrire ce que l'on fait dans ce cas.}
%
...
address, it raises a so-called Page Fault \GH{Pourquoi des majuscules?}.
...
components remains challenging, for various reasons.
...
\thomasrk[inline]{J'en viens à me demander si on ne peut juste pas virer la
  ...
  \GH{Je pense que çà serait mieux en effet. Tel quel, çà vient un peu comme une cheveu sur la soupe} 
%
...
the challenge posed by illegitimate firmware modification. These approaches have all in common to aim at enforcing the correct
...
%
This prevents the system software from modifying with \GH{modifying with?} the content of the Flash
...
dedicated execution mode of x86 processor (\ref{subsec:usecase:hse:smm}) \GH{cette référence est cassée}.
%
...
successfully tamper with the Flash Memory \GH{parfois tu utilises des majuscules pour Flash Memory et des fois, non. Fais une passe d'homogénéisation. Sans majuscule, si tu veux mon avis...} content.
%
The attack proceeds as follows:
...
memory, by making the processor the new root of trust. \GH{Je ne comprends pas "making the processor the new root of trust". Il faudrait donner un peu plus de contexte pour qu'on puisse comprendre cette affirmation}
%
