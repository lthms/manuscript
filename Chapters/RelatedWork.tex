...
definition of \ac{hse} mechanisms and experiment on using this formal definition
...
alternative to model checking\thomasrk{refs}, and a successful solution to
...
Our formal definition of \ac{hse} mechanisms makes a clear distinction between
...
formal definition ---that is specifying requirements and verifying they fulfill
...
%
First, we discuss the limits of labeled transition systems in regard to our
objectives (\ref{subsec:sota:ioauto}), and how process algebra systems have been
used to overcome these limitation in related work (\ref{subsec:sota:palgebra}).
%
We then explain how we take advantage of a functional programming language
formalism called programs with effects and effect handlers as a foundation for a
compositional reasoning framework for Coq which addresses our objectives
...
\begin{example}[Airlock System as Interface Automata]
  ...
  is to handle requests coming from users and open and close doors in
  consequence.
  %
...
a simple ISA semantics and Lamport's sequential consistency \thomasrk{ref
...
Our formal definition of \ac{hse} mechanisms is reminiscent of the
...
\GH{Il faudrait faire ici une conclusion rapide qui résume les deux problèmes
  auxquels tu t'es attelé et fasse la transition avec tes deux parties
  suivantes. Rapide, concis, claire ;-)}
