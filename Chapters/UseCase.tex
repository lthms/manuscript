...
semantics of several x86 instructions it implements\thomasrk{ref sgx
  explained?}--- is partly determined by the microcode it has loaded.
%
...
\ac{pch}\thomasrk{ref}, which handles slower hardware components (\emph{e.g.}
hard drives, USB devices, PCI devices, etc.).
%
...
%
Indeed, cores have their own address translation mechanisms, namely segmentation
and pagination\thomasrk{ref}, which are configurable by the software components.
...
%
The preferred access control model in this context is Access Control List
(ACL)\,\thomasrk{ref}.
%
...
one of the numerous hardware timers (\emph{e.g.} PIT\thomasrk{ref}, APIC
...
% been leveraged to \emph{e.g.} retrieve secrets\thomasrk{ref} or defeat software
...
standardize the boot sequence process in order to favor interoperability of
...
implemented by the \ac{bios}--- focus on the \emph{how} rather than the
\emph{what} and \emph{why}\thomasrk{ref ref ref \\o/}.
%
...
security vulnerability\thomasrk{ref smram + intel bios security}.
%
...
%
As for the \ac{bios}, Intel provides the so-called \ac{smm}, introduced in the
Intel manual as follows:
...
For reference, the Table~\ref{tab:usecase:smramc} reproduces the related section
of the processor datasheet.

\begin{table}
  \begin{tabular}{c}
    TODO
  \end{tabular}
  \caption{The SMRAMC register specification}
  \label{tab:usecase:smramc}
\end{table}
...
or, for most recent versions, in dedicated internal registers\thomasrk{ref}.
%
...
\texttt{0x34}, which is incremented by a \ac{cpu} everytime it handles a
\ac{smi}\thomasrk{ref volume 4}.
%
...
end\thomasrk{ref?}, whose implementation may differ from one product to another.
...
In 2015, Xeno Kovah \emph{et al.} have shown \thomasrk{showed? à vérifier et
  répercuter sur tout le chapitre} it was possible to leverage the Intel
TXT technology to circumvent the flash memory lock-down
...
\ac{smi} in the TXT Software Development Guide as ``most systems will not work
properly if \ac{smi} are disabled for any length of time.''\thomasrk{ref}
