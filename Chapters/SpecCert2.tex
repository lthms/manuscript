...
We emphasize that there is many valid definition of the \func{context} function,
...
SMRAM and $\func{strat}$ tells which cache strategy has to be used when the core
is in \ac{smm}.
...
\( \func{Write}(\var{pa}, v) \) takes into account
%
\begin{inparaenum}[(1)]
...
\item whether the core is in \ac{smm} or not,
...
%
However, we defined {\scshape Minx86} with extensibility in mind.
%
...
%
In this section, our objective is to give a formal definition of the \ac{hse}
mechanism implemented by the \ac{bios} (denoted by \( \Delta_{\mathtt{bios}} \))
to remain isolated from the rest of the software stack
(\ref{subsec:speccert2:bioshsedef}), and to verify its correctness with respect
to the \ac{bios} code injection sub-policy \( I_{\mathtt{bios}} \) (Definition~\ref{def:speccert:biospol})
