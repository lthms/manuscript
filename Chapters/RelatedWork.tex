...
%
For example, a notorious\PC{well-known?} inference rule is the \emph{Modus ponens}, which states
...
verify these proofs (supposedly in a more reliable way than
humans).\PC{parenthèse: subjectif (et sarcastique)}
%
Therefore, the quality of a theorem prover is measured by its capacity to reject
ill-formed proofs.
%
Modern theorem provers are built as ``certified''\PC{emph ? ou c'est encore du
sarcasme ?} layers around a certification as small and simple as possible.
%
...
checking\,\cite{clarke2018modelc} are two notorious\PC{décidémment \ldots}
	classical instances of this approach.
...
%
Temporal logic forms a family of modal logic, such as Linear Temporal Logic
(LTL)\,\cite{sistla1985ltl} or Computation Logic Tree
(CTL)\,\cite{clarke1981ctl}.
%
They\PC{They = ? Les opérateurs sont définis après} allow for reasoning about propositions over time.
%
