...
\[
  \func{safety\_property}_{P}(R) \triangleq \forall \rho \in R, \forall
  (h, ev, h') \in \mathcal{T}(\rho), P(h, ev, h')
\]
...
\begin{definition}[HSE Mechanism]
  ...
  A \ac{hse} mechanism is specified with a tuple
  $\Delta = \langle \Sigma, S, T, \func{context}, \func{inv}, \func{behaviour}
  \rangle$, such that
  %
  \begin{itemize}
...
satisfies the state requirements (\func{inv}) and each software transitions of the run
satisfies the software requirements (\func{behaviour}).
%
...
\begin{theorem}[Correct HSE Mechanism for Safety Properties]
  ...
  \[
    ...
    \Rightarrow P(h, l, h')) \Rightarrow \Delta \sqsubseteq
    \func{safety\_property}_{P}
  \]
