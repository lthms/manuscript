...
\[
  ...
  \setshortdef{\var{true}, \var{false}},\quad\func{tag} :
...
%
\[
  \func{phys\_to\_hard} : \texttt{MC} \times
...
%
\[
  \func{phys\_to\_hard}(\var{mc}, \var{in\_smm}, \var{pa}) \triangleq
  \begin{cases}
    \textsc{Vga}(i) & \text{if }\var{in\_smm} = \val{false}\text{, }pa \in
    \texttt{pSmram}\text{,} \\
    & \quad\text{ and } \var{mc}.\func{d\_open} = \var{true}  \\
...
%
\[
  \func{phys\_to\_hard}(m, b, pa) \neq \func{phys\_to\_hard}(m', b, pa)
\]
...
Let \func{pre} be a predicate over \( L \times H(S) \), and \func{post} be a
predicate over \( L \times H(S) \times H(S) \), then the transition relation of
