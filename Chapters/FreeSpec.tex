...
\noindent
%
For many years, industrial vendors\,\cite{reid2017arm,leslie2015sgx} \GH{la
  deuxième référence est cassée} and
...
composing them to conclude about the properties of the system as a whole. \GH{Tu
  parles de plusieurs requirements mais tu n'en n'évoque qu'un seul. En outre,
  cette intro est longue et parfois un peu maladroite. Je la réduirais en une ou
  deux phrase plus sobre et directe du style 'Modéliser de manière préciser un
  système complexe que représente une architecture x86, constitué de plusieurs
  composants interagissant entre eux, nécessite une approche modulaire.'}
...
when experimenting on {\scshape Minx86}, and discuss how we propose to take our
approach a step further (\ref{subsec:freespec:approachoverview}).
%
Finally, we give an overview of our compositional reasoning framework key
concepts (\ref{freespec:subsec:freespecoverview}).
...
used it for.
%
Our decision to embed the memory location ownership directly inside the model,
along side with the concrete content of each memory location, remains a mistake
with respect to our objective to rely on general-purpose model to specify and
verify \ac{hse} mechanisms.
%
To continue with a similar logic would mean modifying the model each time we
want to reason about a security policy which requires additional information.
%
For instance, if we want to be able to determine how many times memory locations
have been updated since the beginning of the trace, we would have to modify the
definition of \( H(S) \) and of the transition relation to add a counter along
with the owner of a memory location.
%
Modifying the transition relation will likely break all proofs previously
written, and therefore this greatly reduces the opportunity to reuse {\scshape
  Minx86} for other security policies and contradicts our initial goal to verify
\ac{hse} mechanisms of the same architecture against the same general-purpose
model.
...
%
We now detail the approach we used, and we explain how we aim to take its key
...
This approach proved to be valuable to deal with the important number of cases
to cover, when used in conjunction with a recurring pattern we
call ``\texttt{remember};~\texttt{destruct};~\texttt{assert};~\texttt{apply}'',
after the Coq tactics.
%
...
%
However, they have proven to be cumbersome to use manually.

The main objective of this second contribution is to automate as much as
possible the
\texttt{remember};~\texttt{destruct};~\texttt{assert};~\texttt{apply}, in a way
that enables formally reasoning about the temporary violation problem.
%
Besides, we want to propose a general-purpose formalism that allows for never
considering the hardware architecture as a whole, but rather focus on
interactions between neighboring components.
%
In practice, most of the work related to mitigate impact of modifying the state
definition was focused on the \texttt{src/x86/Architecture} files.
%
Our last objective is to propose a modeling approach which favors the separation
of the security policy and the hardware model definitions.

To that end, we take the principle of our theory of \ac{hse} mechanism a step
further.
%
Our theory is based onto a clear separation between hardware components from the
one side, and software components from the other side; software components can
interact with hardware components through the execution of instructions, modeled
by the software transitions of the hardware model.
%
In this Chapter, we propose a general-purpose framework that generalizes this
interface-driven approach for the hardware components.
...
The specification and verification work in FreeSpec is centered around the key
concept of \emph{interfaces}.
%
...
\begin{example}[{\scshape Minx86} as a component-based system]
  Figure~\ref{fig:freespec:interfacedriven} pictures how this could materialize
  as an alternative model to {\scshape Minx86}.
  %
...
so-called abstract specifications which allow for defining requirements over how
the interface should be used and requirements over the interface operations
results.
...
\begin{example}[BIOS Isolation]
  ...
  the interfaces exposed and used by the memory controller, as pictured in
  ...
  users, and over the interface results.
  %
  \begin{itemize}
  ...
  core (which executes the software components) and a cache.
  %
  ...
  %
  We proceed similarly, by defining a second abstract specification, this time
  against the interface of the cache.
  ...
  %
  However, contrary to the memory controller, trying to prove the correctness of
  the cache with respect to this second abstract specification for a cache that
  lacks the SMRR would uncover the SMRAM cache poisoning.
\end{example}
...
%
We proceed with the following steps: modeling the component in terms of
