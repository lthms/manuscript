...
\noindent
%
In the previous chapter, we have introduced a theory of \ac{hse} mechanisms,
whose purpose is to be the foundation of a three-step methodology to formally
...
environment to the \ac{bios}.
%
It relies on the System Management Mode (SMM) and the System Management
Interrupt (SMI).
...
statements in SpecCert, so the interesting reader familiar with Coq can refer to
our machine-checked proofs.
...
of a processor, a \ac{dram} memory cell, etc.
%
...
fetched by the processor in order to be decoded and executed.
...
$s$ was fetched in order to be executed by the processor during a transition
...
\begin{example}[Application Code Injection]
  ...
  processor starts the execution of the application in ring 3, it executes
...
core, a cache, a memory controller, a \ac{dram} controller and a VGA
controller\,\footnote{A VGA controller is a hardware device which on we can
  connect a screen. It exposes some memory to the core for communication
  purposes.} which both expose some memory to the core.
...
%
\TODO{Le lien de cause à effet entre ces deux phrases n'est pas évident. En
  outre "a HSE mechanism" est un peu vague. En l'état, ton modèle a été défini
  pour pouvoir spécifier et vérifier le HSE lié à la SMM. Il vaut mieux être
  précis dès le début.}
%
...
%
Among them, the \ac{smrr} are used to tell the core where the SMRAM is and which
...
\TODO{La nuance entre "written back to the memory" et "content of a memory
  location is overwritten during a transition " n'est pas très claire. Dans le
  cas d'un write back, par définition le contenu de la mémoire est modifiés...}
...
\TODO{Suite à notre discussion téléphonique, je comprends que tu n'as pas pu
  définir avant précisemment context car celui-ci dépend du raffinement de ton
  approche générique. Mais ici tu devrais pouvoir le définir précisemment (en
  t'appuyant sur les trois règles citées précédemment. Hors tu ne le fais
  pas...}
...
\TODO{Pourquoi Cache et Mem sont paramétré par S et pas les autres? Tu devrais
  l'expliquer brièvement ici}
...
\paragraph{Core}
%
The set of states of the \ac{cpu}'s core is denoted by $\texttt{Core}$.
%
...
%
%
The boolean $\func{in\_smm}$ is set to $\val{true}$ when the \ac{cpu} is in
\ac{smm} and to $\val{false}$ otherwise.
%
...
%
The set of states of the \acp{smrr} \TODO{SMRR est noté ici en majuscule mais la
  variable censé les modélisé est en minuscule.} is denoted $\texttt{Smrr}$.
%
\[
...
%
\TODO{Cette phrase est un peu ambigu. On pourrait comprendre que n'importe quel
  softare peut lire n'importe quelle adresse physique. En pratique ce n'est pas
  le cas car un mécanisme de mémoire virtuelle, utilisant la pagination et ou la
  segmentation, va s'interposer et empêcher la lecture de certaines adresses
  physique. Par contre, une fois cette traduction effectuée, rien n'empêche la
  lecture d'une adresse physique (le MC?). Il faudrait le rappeler ici.}
%
...
%
\TODO{En l'absence de plus de détail (formalisme), difficile de comprendre
  précisemment cette dernière phrase}
%
...
%
\TODO{Idem remarque précédente. Difiicile de comprendre ce que tu veux dire sans
  le modèle explicite sous les yeux.}
...
%
\[
  h \xrightarrow[\func{context}]{\func{Fetch}} h' \triangleq h
  \xrightarrow[\func{context}]{\func{Read}(h.\func{pc})} h'
\]
%
...
%
\TODO{Un peu rapide. Il faudrait commenter un peu cette définition. En outre,
  figure est-il un environnement adéquat pour ce genre de définition? (on
  s'attend à une figure, pas des définitions logiques...). Peut-être faudrait-il
...
%
\TODO{Il manque une transition avec la suite. Une annonce du plan "local",
  notamment.}
...
\TODO{Je comprends ta volonté de résumer une preuve surement un peu rébarbative
  à décrire et surtout de ne pas réécrire ce que tu as fais en Coq. Toutefois,
  sans la lecture du code Coq, difficile de comprendre et d'apprécier
  véritablement ton travail. C'est à mon avis un problème. Ton manuscript
  devrait expliquer plus précisément ce que tu as fais en Coq pour qu'un lecteur
  averti puisse juger ton travail sans aller lire ton code. Il faudrait demander
  l'avis à des spécialistes (Yann?)}
...
%
\TODO{Je ne comprends pas cette phrase. Il y a trop d'implicite. Il faut que tu
  développes un peu ton raisonnement.}
%
Unfolding the definitions previously introduced in this Chapter and its
predecessor
%
\TODO{Arrête d'utiliser des qualificatifs relatifs pour désigner les choses
  comme "previously introduced" ou "its predecessor". Il y a beaucoup de
  concepts (définition, théorèmes, etc.) dans ton manuscrit donc à la fin on est
  perdu! Numérote systématiquement les concepts (déf, etc.) et nomme les
  explicitement "Unfolding definitions xxx".} leaves us to prove that \TODO{Je
  n'arrive pas à suivre. Je ne comprend pas pourquoi prouver la correctness se
  réduit à prouver la formule pour fetch seulement. Il faudrait prendre le temps
  d'expliquer.}
%
\[
  \begin{array}{l}
...
%
\TODO{Une fois de plus, la présentation de la preuve est un peu rapide et
  difficile à comprendre sans plus d'élément.}
...
\TODO{Je n'aime pas trop le "perspective". On aurait tendance à penser que c'est
  du future work. A la lecture de ce qui suit, c'est un peu plus qu'une simple
  perspective.}
...
is the complete software stack. \TODO{Je pense qu'il faudrait rappeler ici
  brièvement le concept : un ensemble de composants logiciels qui sont ordonnés
  par une relation d'ordre partiel (forme un treillis?) et I est la politique
  d'isolation globale.}
%
...
\TODO{Il faudrait développer un peu la transition entre le paragraphe précédent
  et ce paragraphe. On doit implémenter plusieurs HSE "concurrently", OK. Et du
  coup, quels sont les problèmes que cela pose et qui justifient la suite?}We
first introduce a \ac{hse} mechanism composition operator \( \sqcap \)
...
\TODO{Même remarques que précédemment, sans le code Coq, difficile de suivre et
  surtout d'évaluer ce qui a vraiment été fait. Il faudrait au moins mentionner
  que cette preuve a été réalisée en Coq (dans quel fichier, ligne, etc.}
...
defined and verified \TODO{A quoi renvoie "previously defined and verified"?
  Stop aux qualificatif relatif, nomme explicitement les choses!}, the \ac{bios}
participates in enforcing \( I \).
%
...
execution of applications \TODO{attention, je comprends en lisant la suite que
  "applications" à un sens bien précis ici. Tu supposes que ta software stack
  est de la forme BIOS>OS>Appk. Il faudrait le rappeler (ou l'énoncer) juste
  avant.} with respect to \( I \) \TODO{que veut dire précisément "constrain the
  execution of app with respect to I'?} verifies this property.
%
We formally specify this additional security policy \TODO{plutôt que additionnal
  secutiy property, il faudrait dire explicitement que c'est la propriété mise
  en oeuvre par ce HSE complémentaire.}
%
\[
...
\begin{proof}
  ...
  execution isolation policies. \TODO{Que sont les "global et partial execution
    isolation policies"?}
  %
  \hfill \( \square \)
\end{proof}
...
separately.  \TODO{Cette dernière phrase est un peu maladroite. Globalement,
  cette section est mal située, je trouve. Finalement, nous n'avons pas besoin
  de la définition préciser de $\Delta_{\mathtt{bios}}$ pour comprendre ces
  problématiques de composition. En revanche, tu fais souvent référence à des
  concepts définis en section 5.1 ou avant... Les idées sont intéressantes mais
  il faudrait mieux les vendre.}
...
\thomasrk[inline]{On doit remettre (séparément) l'argumentation du « ça
  bénéficie aux deux mondes » de manière plus concrète.}
%
\TODO{Je suis en phase avec l'argumentaire développé dans l'abstract
  (firstly... et secondly...). Tu devrais le reprendre (en le développant) ici
  où l'arguentaire est moins claire et ne semble pas "calqué" sur sa première
  énonciation (il devrait).}
...
%
\TODO{Phrase trop longue et difficile à suivre. Couper en deux. Première phrase
  "on a proposé une approche pour spécifier et vérifier un HSE au regard d'un
  model hardware (virer architecture). On a ensuite appliqué cette méthodo sur
  un exemple concret : le mécanisme d'isolation du BIOS utilisant la SMM.}
%
Such approach can benefit both hardware designers and software developers.

First, the hardware model \TODO{A quoi renvoie précisément "hardware model" dans
  ton approche? harware\_req? software\_req? les deux? Il faut être explicite }
can be used as a formal specification by software developers. \TODO{tu es sur
  que c'est bien "software dev" que tu as voulu mettre ici? Dans ce cas je ne
  comprends pas trop ton développement. Tu dis que ton approche peut à la fois
  bénéficier au hard designer et soft dev mais ton "first" et ton 'secondly"
  s'adresse tous deux au "soft dev"...}
%
...
not specified if the first statement is true\,\footnote{If we had to actually
  ...
  correct one.}. \TODO{Cette phrase et la footnote correspondante ne sont pas
  claires.} \formatLTS{Minx86} as a formal specification does not suffer from
the same flaw. \TODO{Mouais, disons que par définition, une specification
  formelle ne peut être ambigu. Dans ton cas, tu as du faire un choix si j'ai
  bien compris. Mais est-ce le bon....?}
%
...
the main advantage of our approach. Thanks to this separation, different
\ac{hse} mechanisms can be studied against the same hardware model.
%
...
model impracticable in practice. \TODO{Que veux dire "monolithic model"? Il
  faudrait détailler un peu plus la problématique}
%
