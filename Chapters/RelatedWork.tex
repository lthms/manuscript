...
against a specific model. \GH{cette phrase est peut-être un peu forte par rapport à ce que tu as fait réellement. Je propose de la supprimer}
%
...
large-scale model that would optimize the applicability of our approach. \GH{plutot que "compromise" qui donne l'impression de s'excuser, je dirai "on a proposé un modèle qui couvre les features dont on a besoin. La specification de ce modèle nous a permis d'identifier les propriétés nécessaire "for a
...
general-purpose model with a large scope with respect to hardware components. \GH{plutot que large scope (ce qui est discutable) tu pourrais insister sur l'interraction entre plusieurs composants. Les travaux d'Intel, de ce que je comprends, se sont plus intéressé à des propriétés "locales" à un composant. En outre, tu t'intéresse plus à la sécurité qu'à la spec fonctionnelle}
%
...
significant changes prior to using it. \GH{mouais, un peu vague. Il faut que tu soigne le positionnonement sur ce travail car c'est l'un de tes reviewer.}
%
% \GH{pas d'autres limitation? Ca fait un peu léger. Peut-être que le modèle est
% trop spécifique aux hyperviseurs?}
...
\GH{l'approche B arrive un peu comme un cheveu sur la soupe. Il faudrait une petite phrase d'accroche qui justifie pourquoi tu vas nous parler de çà. Peut-être un truc qui dit que l'approche qui a le plus étudié la composition est la méthode B, ou un truc du genre. En outre, B n'apparait pas dans la section précédente sur les outils. Peut-être est-ce par ce que B n'est qu'une méthodologie et que plusieurs ouils peuvent être utilisé. Il faudrait le rappeler.}
Several
...
means of a theorem prover. \GH{la fin de la phrase "set the bar high for compositional reasoning by
means of a theorem prover." est bizarre. Il faudrait a minima faire 2 phrases et expliciter un peu la seconde.}
...
\GH{il manque ici une mini conclusion sur B et une transition plus marquée vers ce qui suit. }
...
monads\,\cite{wadler1990comprehending,jones2005io}. \GH{Dans ce qui suit, tu
  supposes que tout le monde sait ce que sont les monades. J'en doute. En outre,
  tu as été super pédagogique avec ce qui précède (et qui est finalement assez
  éloigné de ton approche) et tu vas très (trop) vite sur la fin, alors que ce
  sont les travaux les plus proches! Tu pourrais par exemple utiliser ton
  exemple de air-lock}
...
Monads have a reputation not to compose very well\,\cite{hyland2006combining},
despite constructions such as monad transformers\,\cite{liang1995mtl}. \GH{En fait, ce qui n'est pas très claire dans ce passage, c'est que cette sous-section est censée parler de "compositional Reasoning for theorem prover". On ne comprends donc pas pourquoi tu nous parles d'un truc (les monades) "[that] have a reputation not to compose very well"}
%
Recently, algebraic effects and effect handlers\,\cite{bauer2015effects}
%
\GH{Idem que pour les monades. Il faudrait expliquer un peu ce que sont les
  effets algébric et les effect handlers}
%
...
A general-purpose x86 hardware model poses an important challenge due
...
has not been widely explored in Coq. \GH{il manque une phrase pour positionner ton travail sur freepsec et annoncer ce que tu vas présenter en section 6, comme tu l'a fait dans le paragraphe précédent pour ce qui relève de speccert.}
