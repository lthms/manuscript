...
\begin{definition}[Computing System]
  ...
  \begin{prooftree}
    \AxiomC{$\func{precondition}(h,ev)$} \AxiomC{$\func{postcondition}(h,ev,h')$}
...
\begin{definition}[Software Execution Tampering]
 ...
 \begin{definition}[Software Execution Isolation]
  ...
  \begin{definition}[HSE Laws]
    ...
    \begin{enumerate}
    \item \func{behavior} preserves \func{inv}: $\forall \transitionLTS{Ez}{h}{ev}{h'}$,
...
\begin{definition}[Compliant Runs]
  ...
  \[ \rho \in \mathcal{C}(\Delta) \triangleq \func{inv}(\func{init}(\rho))\,\wedge\,\forall
...
We now define $\func{behavior_{Smm}}$. We only define two
