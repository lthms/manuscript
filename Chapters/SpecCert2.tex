...
%
Firstly, \( \sqcap \) forms a commutative monoid, meaning it is both associative
and commutative.
%
As a consequence, the order in which we construct a ``global'' \ac{hse}
mechanism from a list of sub-mechanisms is not important.
%
...
\ac{bios} to isolate itself from the rest of the software stack and
$\Delta_\mathtt{os}$ primarily implemented by the OS to constrain the execution
of the applications, that is
  %
\[
  \begin{array}{cl}
    \Delta_{\mathtt{bios}}
...
$\Delta_\mathtt{os}$ satisfies the software execution isolation policy $I$, that
is
  %
\[
...
\subsection{HSE Mechanisms Compatibility}
...
%      \var{strat} \} \)}%
%    \BinaryInfC{\( h \xrightarrow[\func{context}]{\func{SetCacheStrat}(\var{pa},
%        \var{strat})} h' \)}
%  \end{prooftree}
%\end{minipage}
%  %
...
%    \BinaryInfC{\( h \xrightarrow[\func{context}]{\func{UpdateSmrr}(\var{smrr})}
%      h' \)}
%  \end{prooftree}
%\end{minipage}
%
