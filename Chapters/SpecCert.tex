...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \node (A1) {app$_1$};
        %
      \node [right=of A1] (A2) {app$_2$};
        %
      \node [right=of A2] (Aetc) {$\dots$};
        %
      \node [right=of Aetc] (An1) {app$_{n-1}$};
        %
      \node [right=of An1] (An) {app$_{n}$};

      \node [below=of Aetc] (OS) {os};
...
% We can derive a security policy (Definition~\ref{def:speccert:secpol}) thanks
% to the function \func{safety\_property}
% (Definition~\ref{def:speccert:safetyprop}).
%%
% According to Theorem~\ref{theorem:speccert:correcthse}, a HSE mechanism
% $\Delta = \langle S, T, \func{context}, \func{inv}, \func{behaviour} \rangle$
% is correct with respect to a Software Execution Isolation Policy
% $I = \langle S, T, \func{context} \rangle$, that is
% $\Delta \models \func{safety\_property}_I$, when
%%
% \[
%   \begin{array}{l}
%    \forall (h, l, h') \in \mathcal{T}(\Sigma), \\
%    \qquad \func{inv}(h) \\
%    \qquad \Rightarrow (l \in L_S \Rightarrow \func{behaviour}(h, l)) \\
%    \qquad \Rightarrow I(h, l, h')
%  \end{array}
%\]
%%
%In other words, as long as trusted software components correctly implements the
% HSE mechanism, untrusted software components cannot tamper with their
% execution.
