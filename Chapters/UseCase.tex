...
%
In most case, it is a proprietary software, and the computer owner has little
control over it.
%
...
%
The question whether this model benefits the x86 computing platform is outof the
...
%
Beside, the flash memory cannot be the only memory dedicated to the \ac{bios}.
%
Memory accesses which target the flash memory are slow, as the latter is
connected to the PCH, behind the Memory Controller.
%
...
\section{BIOS \ac{hse} Mechanism and Architectural Attacks}
\label{subsec:usecase:hse}
...
instructions.
%
Contrary to the common believe, x86 execution modes are not organized in a
...
\begin{quote}
  ...
  OEM-designed code.
  %
  It is intended for use only by system firmware, not by applications software
...
%
This design has been made to help \ac{bios} initializes the SMRAM content.
%
...
the flash memory.
%
This second access control mechanism is implemented by the PCH.
%
...
%
When the \texttt{BIOSWE} bit is clear, the PCH only authorizes read accesses to
the Flash Memory.
%
If the \texttt{BIOSWE} bit is set by a \ac{cpu}, the behaviour of the PCH
depends on the value of the \texttt{BLE} bit.
%
If the \texttt{BLE} bit has been set by the \ac{bios} during the boot sequence,
then the PCH triggers a \ac{smi}.
%
...
Memory, even if the PCH now authorizes write accesses.
%
...
\ac{bios} update.
%
The system software sets the \texttt{BIOSWE} bit to notify the \ac{smm} code
...
\subsection{SMRAM and Cache Poisoning Attack} % -------------------------------
...
\item Attackers issue a memory write access targeting the SMRAM, and because of
  ...
  cache, but does not pass the access on the Memory Controller.
%
...
%
the former keeps copies of successful accesses to decrease latency due to memory
accesses.
%
...
means it requires an additional step of configuration to tell the cache where
the SMRAM is.
%
...
\acp{cpu}, whose aim is to reduce the \ac{tcb} of a system software, by removing
...
%
With TXT, the goal was to reduced it to the \ac{cpu} itself, which was acting as
...
disabling \acp{smi}.
%
In the context of TXT, this might makes sense, as the \ac{bios} is considerer
untrusted.
%
...
version of TXT was clearly misconceived with regards with the existing state of
the hardware architecture.

\subsection{Speed Racer}
\label{subsec:usecase:hse:speed}

There is at least another attack which have defeated the Flash Memory Lock-down
protection.
%
In 2015, Corey Kallenberg \emph{et al.} have shown that the scenario detailed
previously, where setting \texttt{BIOSWE} was triggering a \ac{smi} to suspend
the execution of the system software, suffered from a race condition in presence
...
will \emph{eventually} enter in \ac{smm} when a \ac{smi} is triggered.
%
...
\item One \ac{cpu} tries \emph{ad infinitum} to overwrite the content of the
  Flash Memory. Because the \texttt{BIOSWE} bit is initially clear, the PCH
  discards its attemps, and the Flash Memory content is correctly protected.
%
...
\item A \ac{smi} is triggered, but by the time it propagates to the first
  \ac{cpu}, the latter may have successfully tamper with the Flash Memory
  content.
%
\end{enumerate}
...
Write Protection).
%
If the \texttt{SMM\_BWP} is set, the PCH discards any write access which targets
