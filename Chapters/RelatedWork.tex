...
systems.
%
Over time, many different definitions of transition system have been
...
approaches.
%
On the contrary\PC{au contraire de quoi ? la suite semble générique}, we explain how a system can be specified using a generic
...
To the best of our knowledge, the more straightforward definition\PC{ce début de
	phrase laisse penser que 1. tu n'es pas sûr 2. il n'y a pas de def
standard ? - à revoir} of a labeled
...
\PC{l'exemple Airlock Systems tombe carrément du ciel, on ne voit pas pourquo
	tu le choisis. Je suggère de mettre ici une phrase disant que tu vas
prendre un exemple récurrent pour illuster, et pourquoi tu le prends}
...
\begin{example}[Airlock System]
  ...
  \item A door of the system can be either \( \mathtt{open} \) or
    \( \mathtt{close} \).
    %
    The set of states of the airlock system reflects the combinatronics\PC{???} of the
    doors states.
  \item A transition is characterized by the opening (\( \mathtt{Open}_i\), with
    ...
    %
  \item The system does\PC{should?} not allow the simultaneous opening of both doors, as
...
notorious\PC{pas notorious (négatif). ``classical'' ?}
