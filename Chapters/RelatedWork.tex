...
ill-formed proofs.
%
Modern theorem provers are built as certified layers \GH{ca veut dire quoi "certified layers"? Que ces couches ont été prouvée avec le theorem prover?} around a trusted kernel which is kept as small and simple as possible.
%
...
Another common approach \GH{a quoi renvoie "another approach"? Theorem prover? } is to rely on an algorithm whose correctness has been
...
\GH{Je trouve que ce tour d'horizon des méthodes formelles est un peu rapide. Il gagnerait à être plus complet et mieux structuré.}

\subsection{Conclusion}

We have implemented our proofs of concepts in the Coq proof assistant.
%
Our choice is motivated by the following reasons \GH{Comme on te l'a déjà dit, il faudrait que le choix de ton approche soit distillé au fur et à mesure. La conclusion ne devrait être qu'une synthèse.}:
%
\begin{itemize}
\item {\scshape Gallina}, the specification of Coq, is an expressive formal
  ...
  type system \GH{c'est quoi un "type dependent system"?}, while {\scshape Ltac}, the proof tactic language of Coq, enables
  powerful automation features. \GH{vu que Coq va être central dans ton approche, tu devrais le décrire un peu plus en détail (lui consacrer un chapitre).}
  %
  This suits well for specifying security properties to be enforced by HSE
  mechanisms, as they can take many forms. \GH{Cette affirmation est un peu gratuite. Tu devrais développer un peu cette idée et montrer en quoi les propriétés de sécurité nécessite une logique d'ordre supérieur (donner des exemples...)}
\item Model checkers and similar automated tools remain subject to the so-called
  ...
  only considers three cache locations). \GH{En pratique, l'implémentation aura bien un nombre limité de coeurs, un espace d'adressage limité... Donc est-ce réellement un problème? En outre, est-ce que cela serait une limitation pour ton approche telle qu'elle est utilisée actuellement ou bien est-ce un argument pour le futur?}
  %
  ...
  arbitrary number of similar components\,\cite{vijayaraghavan2015modular}. \GH{et? il faut remettre cet argument dans le contexte de ton travail. En quoi cela serait-il utile pour ce que tu fais?}
...
\GH{De manière générale, cette première partie de l'état de l'art est à revoir. Il faut mieux positionner ton travail, de manière plus systématique et "carré"}
...
automata\,\cite{de2001interface} are better suited to model concurrent systems. \GH{et? Quelle structure vas-tu utiliser dans ton approche? Pourquoi?}

\GH{Tel quel, cette sous-section est un peu vague. Elle gagnerait à être un peu plus étoffée et surtout présentée de manière plus rigoureuse, carré. On ne comprend pas vraiment pourquoi tu nous parles de LTS sinon que d'autres le fond alors çà a l'air d'être une bonne idée...}
