...
\noindent
%
For many years, industrial vendors\,\cite{reid2017arm,leslie2015sgx} \GH{la deuxième référence est cassée} and
...
composing them to conclude about the properties of the system as a whole. \GH{Tu parles de plusieurs requirements mais tu n'en n'évoque qu'un seul. En outre, cette intro est longue et parfois un peu maladroite. Je la réduirais en une ou deux phrase plus sobre et directe du style 'Modéliser de manière préciser un système complexe que représente une architecture x86, constitué de plusieurs composants interagissant entre eux, nécessite une approche modulaire.'}
...
%
Algebraic effects and handlers\,\cite{bauer2015effects} overcome this double challenge.
%
...
In this Chapter, we aim to show how a variant of algebraic effects based on Free \GH{pourquoi un F majuscule... }
...
The rest of the paper \GH{Je pense que tu voulais dire "chapitre" ;-)} proceeds as follows. \GH{TOn plan n'évoque pas la section 6.1}
%
...
% ==============================================================================
\GH{Il faudrait une ou deux phrase d'introduction à cette section avec annonce du plan interne. Après lecture de la suite, il me semble que le premier paragraphe de la sous-section suivante correspond à ce que je souhaiterais mais il est mal placé.}
...
definition. \GH{En fait, ce premier paragraphe est un plan de la section. Il devrai donc être placé en dehors de cette première sous-section. Ici, il faudrait une petite phrase d'introduction de la sous section, en présentant succintement les deux limites que tu vas développer. }
...
sequence. \GH{A mon avis il manque des mots dans cette phrase pour qu'elle soit compréhensible}
%
...
\]
%
\GH{A quoi correspondent t, s et r? Pourquoi 3 fonctions et pas plus? Ca semble tomber de nulle part...}
...
all at once can quickly become unmanageable. \GH{Pourquoi?}
%
One possible approach to face this challenge is to manually divide the
transition into several change updates ---\emph{i.e.} one per state update--- \GH{je ne comprends pas "several change updates ---\emph{i.e.} one per state update". "change updates" me semble une répétition. Et je ne voit pas à quoi correspond un "state update". }
and prove that \( \iota \) is preserved throughout the transition, as
illustrated in Figure~\ref{freespec:figure:seqproof}. \GH{Quel est le langage utilisé dans cette figure? Galina? Il faudrait le signaler et commenter un peu cet extrait pour qu'il soit accessible au non-spécialiste. Ce n'est pas une mauvaise idée de mettre un tel code mais il faut que la plupart des relecteur puisse comprendre l'idée générale.}
%
Proving \( \Delta_{\mathtt{bios}} \) was correct with respect to
\( I_{\mathtt{bios}} \) using Coq\,\cite{letan2016speccertcode} led us to
successfully adopt this approach. \GH{On est un peu perdu. Tu as adopté cette approche dans SpecCert ou FreeSpec?}
%
In practice, we relied on a recurring pattern we call
``\texttt{remember};~\texttt{destruct};~\texttt{assert};~\texttt{apply}'', after
the {\scshape Ltac} tactics. \GH{Normalement l'objet de ce paragraphe est de décrire une limitation de MinX86. Or là j'ai l'impression que tu as glissé vers une description de ton approche, qui normalement est abordée après. Du coup, ce qui suit n'a peut-être pas sa place ici.} \GH{les termes "remember", "destruct", etc; sont-ils des termes à toi ou bien connu de la communauté Coq? A quoi correspondent-ils? Il me semble qu'il s'agisse de "tactique". Il faudrait le préciser ici}
%
We illustrate the use of this pattern with the proof obligation with the proof
goal \GH{"with the proof obligation withe the proof goal" semble redondant. En tout cas la phrase est étrange.} pictured in Figure~\ref{fig:freespec:postcondition} \GH{la référence de la figure est cassée}.
%
We have extracted this goal directly from the SpecCert project; we have
...
\item Using \texttt{remember}, we can make the sequence of state updates more
  ...
  Figure~\ref{fig:freespec:entrypoint}). \GH{Pour moi qui ne suit pas un spécialiste, cette séquence ne me semble pas plus claire, je ne la vois dans aucun des deux figures. Il faudrait peut-être décrire par une phrase ou deux comment on doit interpréter/lire ces figures.}
  %
  As a result, the Coq goal is more readable, which greatly simplifies the 
...
\item Using \texttt{destruct}, we can explore the alternative paths. 
  ...
  %
  We explore both solution, though two complementary goals \GH{ne faut-il pas un "s" à solution? en outre, je ne comprend pas trop "though two complementary goals". Ne manquerait-il pas un mot?} (see the two
...
\item Using \texttt{apply}, we can leverage lemmas about \texttt{inv}
  ...
  (Figure~\ref{fig:freespec:remember}, line 11) has a lemma called
...
The ``\texttt{remember};~\texttt{destruct};~\texttt{assert};~\texttt{apply}'' pattern 
structures the proof, but it remains cumbersome to use.
%
Besides, it emphasizes another important limitation of {\scshape Minx86}:
it is potentially subject to the so-called \emph{temporary violation
...
\emph{during} the transition. \GH{cette limitation est importante à noter (et elle est plus facile à comprendre telle que tu la décrit par la suite). En revanche, pour moi il s'agit d'une autre limitation que le côté monolytique, qui devrait donc se trouver dans un paragraphe dédié.}
%
For instance, if it exists a state \( h \) such that \( \neg r(h) \) or
\( \neg s(r(h)) \) holds true, then \( M \) is subject to the temporary
violation problem. \GH{Je ne comprends pas la condition car je ne vois pas à quoi correspond r et s}
%
...
\begin{example}[Temporary Violation Problem in an Airlock System]
  ...
  at least one door should be close at anytime, for instance: \GH{la phrase est longue. à couper en deux }
...
forgetting a given state updates \GH{si update est un verbe ici, il manque un complément. Si c'est un nom, il devrai être au singulier} and yet being able to conclude the proof.
%
...
\[
  ...
  \end{array}
\]
\GH{Je ne vois pas l'utilité de rappeler l'invariant de delta bios ici.}
%
...
%
Modelling that threat without changing the modeling structure of {\scshape
...
for instance on configuration stored in memory (\emph{e.g.} a
...
to modify the model another time. \GH{plutôt que 'une autre fois" il faudrait plutôt insister sur la nature et l'importance des changements que cela va nécessiter.}
%
