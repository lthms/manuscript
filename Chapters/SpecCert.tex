...
components, as a particular \ac{lts} (Definition~\ref{def:sota:lts}) \GH{la défintion 3.1 est loin. Si on lit le pdf en version informatique, on clique sur le lien et çà va. Si le texte est imprimé, il faudrait peut-être rappeler la section est la page ce cette défintion).} with two
...
%
We aim to model \ac{hse} mechanisms as traces subsets 	\GH{je pense (mais je peux me tromper) que lorsque tu place un nom devant un autre pour marquer le possessif, ce nom à valeur d'adjectif et qu'il doit être au singulier. Donc "trace subsets" plutôt que "traces subsets". Il faudrait vérifier que tu n'as pas le même souci ailleur}, to discard traces that are
...
purpose. \GH{cette phrase est longue et on ne voit plus trop à quoi se rattache "to find aaitionnal...". S'il se rattache à "cannot be losen without", il faudrait utiliser le gérondif "without ... finding additionnal restrictions.}
...
\begin{example}[Memory Flash Lockdown Compliance]
  ...
  \GH{cet exemple n'illustre que l'état initial. Il faudrait dire en outre que lorsqu'une exécution est exécutée en SMM elle respecter les software req...}
\end{example}
...
%
As it stands, the \ac{hse} mechanism described here satisfies both laws. \GH{pourquoi? Il faudrait expliquer rapidement pourquoi c'est bien le cas.}
%
...
enforced, but the core does not read its content when it needs it. \GH{la fin de cette phrase n'est pas claire. Reformuler}
...
In most cases, the security policy is expected to be enforced in all traces of a
...
all time. \GH{Je vois ce que tu veux dire mais telle quelle, ton explication apporte plus de confusion qu'autre chose. Surtout la première partie. Il faudrait plutôt dire que les architecture matérielle n'étant pas safe par defaut, la politique de sécurité ne s'applique pas à l'ensembles des traces du système R(Sigma). Il faut que le mécanisme hardrware soit correctement initialié et utilisé par le code trusté. Par conséquent, un HSE est correcte si la politique de sécurté est vérfiée sur le sous-ensemble des traces qui vérifent les contraintes de configuration et de bonne utilisation par le code trusté, donc C. }
%
...
\( \mathcal{R}(\Sigma) \). A proof that a \ac{hse} mechanism \( \Delta \) is correct with respect to a
security policy \( P \) rules out the risk of a vulnerability in the
hardware features of the hardware model.
...
can be difficult.
%
In the context of this thesis, we focus on safety properties. \GH{tu pourrais argumenter un peu en indiquant que, de fait, la plupart des HSE implémente une politique de safety}
%
...
A proof of correctness of a given \ac{hse} mechanism asserts that the
...
modifying the instruction responsible to implement it. \GH{ce n'est pas le seul risque (même si c'est le plus important). De manière générale, si un composant non trusté peu influencer le comportement d'un code trusté (par exemple, en perturbant son flot d'exécution), il peut l'amener à ne plus respecter les software req. Mais c'est le problème du software trusté. Il doit être correct ET robuste aux attaques.}
%
...
traces where a given software component prevents a given \GH{"another" plutot que a given} software component to
correctly implement its functional specification. \GH{Cette phrase n'est pas très claire. Il faudrait développer en NOMMANT les choses. Un software component s1....}
%
...
\GH{Cette conclusion intermédiaire est un peu "too much" (pourtant j'aime bien les conclu intermédiaire). Comme elle n'est pas séparée du reste, on la relie à tort à la sous-section 4.1.4. Je serai pour la supprimer (en tout cas, supprimer la sous-section). Eventuellement, reprendre le début (In this section...at the same time) comme introduction de la section suivante. Ca fera une bonne transition avec "We now aim to ..". Dans tous les cas, virer "In the next Section... hse composition" qui est redondant avec ce qui suit.}
...
\begin{definition}[Transition-Software Mapping]
  ...
  of owners of the fetched instructions during this transition. \GH{fetched instruction n'est pas défini clairement.}
\end{definition}
...
with \( l \) from a state \( h \). \GH{Cette phrase est ambigüe. Est-ce que c'est le fetch qui a lieu durant la software transition? J'avais cru comprendre que l'exécution d'une instruction pouvait correspondre à plusieurs "software transition". Du coup, pour chacune des ces transition, fetched renvoie la même valeur? Ou pas? Ce n'est pas claire}
...
\begin{example}[Application Code Injection]
  When a user starts \GH{execute plutôt que start?} a new application, the operating system
...
particular software stack. \GH{virer "in the context  of a particular software stack"? Je ne vois pas trop ce que çà apporte}
...
\begin{definition}[BIOS-OS-Applications Code Injection Policy]
  ...
  processor is not in \ac{smm} and is in ring 0, the system software component \GH{comme tu utilise le terme OS avant dans ta définition, je dirais OS ici plutot que system software}
...
mechanisms against the same reference, both in isolation and in composition. \GH{attention avec ce claim un peu fort. Tu affirmes haut et fort qu'il te faut un modèle générique de x86 sauf que juste après, en section 5, tu vas annoncer que tu as fait un modèle réduit avec seulement les features qui t'intéressaient pour modéliser la SMM...}
%
