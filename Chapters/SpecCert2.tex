...
%
We consider the core \GH{Il faudrait préciser "a cpu core". En outre,
  considères-tu pour l'instant seulement un seul coeur par CPU? Il faudrait le
  rappeler ici explicitement.} can be either in System Management Mode
...
and \( \mathtt{Core} \) will have to be parameterized by \( S \) too. \GH{je virerai ces deux dernières phrases car elles ont tendances à être négatives. En outre, plus loin, on se rend compte que tu modélise PC, qui est bien un registre du processeur! Eventuellement, dire que ton modèle n'a pas besoin de suivre la provenance des contenus des registres (et encore...)}
...
\item If the cache strategy for the address \( \var{pa} \) is set to
  ...
  \]
  %
  \GH{cette formule dépasse un peu de la page. Peut-être peux tu enlever le "= true"}
  ...
  \]
  %
  \GH{on a (j'ai) perdu de vue ce que dispatch signifie. Il faudrait peut-être le rappeler brièvement}
...
\item If we consider a similar case, but this time the address \( \var{pa} \) is
  ...
  executed by the core, its new content is \( v \) \GH{a quoi correspond v? Dans le reste de la phrase, tu désigne explicitement les choses, sauf v, ce qui fait un peu bizarre} and its dirty bit is set,
...
\]
%
\GH{dans la formule, je mettrai les "where..." qui précise la formule, à gauche et je laisserai sur la colonne de droite les "if..." qui préscisent quel cas s'appliqe. En outre, je rappelerais pour le dernier cas qu'il correspond à read\_from\_cache = false}
...
states that the result of this \IO is a predefined \GH{constant? dummy? value} value, hence the function
...
\paragraph{Requirements over States.} \GH{je viens de réaliser que tu as renommé les "hardware requirements" en "requirement over states". Outre que le nom devient long je ne suis pas sur qu'il ne reste pas quelques mentions de hardware requirement dans le manuscrit. Il faut vérifier}
%
...
We do not consider \GH{'we do not consider " fait un peu négatif. En fait, comme tu t'intéresse à isoler le BIOS de l'OS et des appli, ca n'a pas de sens de vérifier pendant le BOOT puisque le BIOS est le seul à s'exécuter pendant ce temps là. Du coup, forcement, la propriété recherché ne s'applique qu'après le boot. En outre, les hypothèse sur l'initialisation sont de même nature que les hypothèse sur le comportement du BIOS à runtime : ce sont des hypothèse sur le logiciel trusté. Il faut le formuler comme çà. Tel que tu l'as formulé, on a l'impression que c'est un cas non couvert.}the boot sequence in this verification problem and the
...
\item The core is configured to read the content of \( pa \) from the cache, and
  ...
  transitions\,\footnote{The interesting reader can have a look at the file
    ...
    \texttt{src/Smm/Delta/Preserve/\{Read,Write\}.v} to see them in action.}. \GH{attention, la référence dans la footnote est cassée}
\end{enumerate}
%
...
%
\( \func{Fetch} \) is a hardware transition \GH{a bon? Ce n'est pas évident à première vue. Et sauf erreur de ma part, ce n'est pas précisé avant. Cela devrait l'être. Tu devrais mieux définir ce qu'est "fetch", quand et comment cet événement arrive... lorsque tu décrit MinX86}, and therefore is not concerned by
\func{bios\_req}. 
%
...
\( h.\func{core}.\func{pc} belongs to the \texttt{pSmram} \). \GH{il y a un problème dans une de tes commandes. Les mots sont soudainement tous attachés}
%
