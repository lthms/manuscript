...
\vspace{1cm}\noindent
%
Our interest in formally verify \TODO{Ou interest in formally verifying? ou alors "our interest in the formal verification of hardware architecture to improve their security"} hardware architecture in order to improve their
security is in line with an ongoing effort \TODO{de qui?}. 
%
...
\subsection{Reasoning about Systems} \TODO{Je trouve le titre peu explicite (contrairement aux autres sous-section.}
...
%
Modelling such a system $C$ \TODO{remarque mineur "a system C" fait bizarre. Je suppose que tu voulais dire "a component C".} is usually done by defining a \ac{lts}
$M \triangleq \langle S, L, \rightarrow \rangle$.
%
$S$ is a set of abstract states which describe the concrete states of $C$. \TODO{C'est quoi un état concret? C'est quoi un état abstrait?}
%
...
\TODO{Je trouve cette sous-section trop vague. Pas de référence. En outre, on ne voit pas trop à quoi elle correspond. Je crois comprendre qu'il s'agit de modéliser le système. Du coup, la question qui se pose est : existe-t-il d'autres modèles que les LTS dans la litérature? Pourquoi ce choix? Peut-on comparer les différentes approches? C'est typiquement ce qu'on attend de la partie état de l'art. Par une vague définission d'un LTS. Je pense que tout le monde voit à peut prêt ce qu'est un LTS. Donc même si rappeler la définition n'est pas un mal, faire une sous-section juste pour çà est trop léger. Tu pourrais en outre donner quelques détail sur "its exact definition often vary
...
correctness of $C$ with respect to certain properties. We first have to formalize these
properties as a statement about $M$, then we exhibit a proof that this statement
is true.
%
This statement is based on the system executions, modelled \TODO{"modelled ou modeled"?} as sequences of
transitions of $M$.
%
We denote $\Sigma(M)$ the set of all the possible executions of $C$, as modelled
in $M$.

\TODO{Remarque générale sur cette section : il manque pleins de référence. Les notions de propriétés, safety, liveness et hyperpropriétés ont été définis dans des ouvrages ou articles de références.}

\paragraph{Properties and Hyperproperties}
%
Certain security properties can be modelled \emph{via} a predicate $P$ on
executions. \TODO{On each execution? D'ailleurs, ne parle-t-on pas plutot de "trace d'exécution"}
%
...
insecure state and no insecure transition occurs. \TODO{Il y a a mon avis des erreurs dans cette phrase. "to require that an execution which starts ... put. Ou alors for an execution ... to put. En outre "from a secure state ... in a secure state". Pourquoi "insecure state". Au final, cela fait une phrase très (trop longue). Tu devrais utiliser une énumération par exemple pour aléger un peu. ou faire plusieurs phrases.}
%
For instance, the BIOS Integrity security property
(Definition~\ref{def:usecase:biosint}) is a safety property. \TODO{Tu devrais expliquer un peu mieux pourquoi}
%
As for liveness properties, they usually require to use a temporal logic which
provides a modal operator to reason about ``eventually''. \TODO{Donne un peu plus de détail. Parle de LTL par exemple. Explique un peu (donne une formule et explique ce qu'elle veut dire) Cite!}
%
For instance, the BIOS Availability security property
(Definition~\ref{def:usecase:biosav}) is a liveness property. \TODO{Idem, explique pourquoi}
...
executions\,\cite{clarkson2010hyperproperties} \TODO{Je ne comprends pas l'expression "is of the form executions"}, that is
%
\[
  P(\Sigma(M))
\]
...
executions is noninterference\,\cite{goguen1982security}, which is used in information flow
analysis.
%
...
the system's observable behaviour remains the same''. \TODO{Tu devrais continuer en donnant un exemple de formalisation du problème et en montrant (intuitivement) que c'est bien une propriété d'ensemble de traces.}

\paragraph{Modelling for Security}
%
The definition of a security property might \TODO{pourquoi "might"? "can?'} be based on information that are not
available to the system $C$.
%
For instance, a x86 hardware architecture does not track information flows \TODO{Je trouve cette histoire de information flow très casse gueule. Comme tu viens de l'évoquer au dessus l'analyse des flux d'information repose sur des hyperpropriétés. Or il ne me semble pas que c'est ce que tu fais par la suite. En fait, tu trace simplement qui est le dernier à avoir écrit dans une zone mémoire, ce qui n'est pas tout à fait la même chose. En outre, à ce niveau du manuscript, le lecteur ne verra surement pas où tu veux en venir (je comprend car je connais ton approche...) }, but
...
result of a read access by the SMM code. \TODO{Il faut expliquer un peu plus pour que le lecteur qui ne connaisse pas tes travaux puisse comprendre. En outre, je pense que cette problématique (ajouter dans le modèle des informations qui sont utile pour vérifier la propriété de sécurité mais qui ne sont pas accessible à l'implémentation, est une technique générique. A mon avis tu dois pouvoir trouver des ref à ce sujet. Par exemple en analyse statique, tu peux typiquement mettre ces informations dans l'environnement.} 
%
...
flow aware'' model. \TODO{Encore une fois "information flow aware'' model. est casse gueule et flou}

This raises questions about model reusability.
%
The x86 hardware architecture includes many different \ac{hse} mechanisms.
%
Defining a specific hardware model for each of them is not an option, yet the security properties they
aim to enforce may heavily differ in terms of model requirements. \TODO{et donc? Je vois ce que tu voudrais dire, mais tu ne le dis pas. Il faut être explicite. En outre, attention, tu es dans l'état de l'art. Or on est en train de tendre vers une description de ton approche. Il faut montrer comment les travaux de l'état de l'art traite ou ne traite pas ce problème. Se positionner!}
...
proving. \TODO{Je ne suis pas un spécialiste des MF mais ce n'est pas un peu réducteur de dire que les deux seules manière de prouver qqc sont le model checking et la preuve de programme? Quid de l'interprétation abstraite? Des problèmes que l'on peut résoudre directement avec un solveur SAT? Peut-être faudrait-il nuancer le propos (les principales approches utilisées dans le cadre xxx) Préciser qu'il y en a d'autres mais quelles sont plus spécifiques/restrictives}
%
...
%
This system is a device made of two doors, and an intermediary chamber. \TODO{Un petit schema serait le bienvenu...}
%
...
property is always true), or \emph{eventually} (the property becomes true at
...
\begin{example}[Airlock System Verification in NuSMV]
  ...
  %
  Because NuSMV does not \TODO{does not have?} a native support for labelled transitions, we embed the
  labels inside the state.
  %
  The value of the variable \texttt{action} at the beginning of the transition
  determines the effect on this transition \TODO{"on this transition" ou "of this transition"?} to the variables \texttt{door1} and
  \texttt{door2}.
  ...
  both doors are close. \TODO{Cette phrase n'est pas très élégante, tu devrais plutôt donner les numéro de lignes}

  Our airlock system is now completely modelled.
  %
  We can specify the safety property we are interested in using the
  \emph{globally} modal operator, denoted \texttt{G} in NuSMV. \TODO{tu devrais donner la formule LTL équivalente, pour t'abstraire de l'implémentation}
  ...
  that it is no longer required for the second door to be closed in order to open
...
\begin{example}[Airlock System Verification in Coq]
  ...
  %
  We can \TODO{use?} {\scshape Gallina} to describe the system in terms of states and actions to
  update this state.
  ...
  {\scshape Gallina} includes a special type, called \texttt{Prop}, to write logical
  ...
  %
  This property is a predicate on the system state, which we can define in {\scshape
  ...
  %
  We write the proof with {\scshape Ltac}, and trust Coq to refuse it in case it is
  incorrect.
  %
...
\TODO{So? Quelle est la conclusion de cette section? Pourquoi va-tu choisir dans ton approche de faire du Coq?}
...
defined in {\scshape Gallina} within the Coq proof assistant are executable, it is
